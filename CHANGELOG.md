...
- The address document is `config.default.json`, fetched from the binary's LINE CONFIG BRANCH (`https://raw.githubusercontent.com/Cork-Technology/cork-cli/config/<major>.<minor>/config.default.json`, a branch that holds only that file; a source run reads `main`; `CORK_DEFAULTS_URL` still overrides). The branch is the no-upgrade escape hatch: a compatible address change pushed there reaches every binary of the line within the hour, while the release tag stays immutable. A key change is breaking and opens a new minor — this rename is why the next cut is 0.7.0-rc.1 and 0.6.0 stays the only 0.6.x binary; the frozen-keys tripwire (`config-frozen-keys.test.ts`) fails offline and, under `CORK_RPC_LIVE=1`, against the live public branch when a known key disappears. `cork-defaults.v2.json` stays on `main` FROZEN for the released 0.6.0, whose binary fetches it from `main` by name: on 2026-09-25 the label rename moved that file's keys on `main`, and `--generation phoenix/v0.4-rc.1` stopped working on 0.6.0 within the hour (reported by the Distribution verifier). A key a released line knows can no longer disappear from the file it reads. The disk cache is `~/.cache/cork-helper-cli/config.default.json`. Policy: cork-knowledge `versioning-and-release.md` R5c.

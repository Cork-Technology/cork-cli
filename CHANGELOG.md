...
Pool-scoped reads, prepares and event scans now follow the generation the POOL lives on, not the chain's primary (0.6 stage 2c). Every pool the venue serves today lives on an older pool manager than the 10-field primary, so a read or bundle routed through the primary's addresses answered for the wrong contract.

### Added

- **`generation` on every pool-scoped result.** `cork_query` cork-pool / account-state / pool-whitelist, the three chain `cork_compute` kinds, `cork_track` marketRef, the 13 `cork_prepare_phoenix` actions and the ForSelf twins resolve the pool's generation from the chain — one batched `shares(poolId)` read across every configured pool manager — and carry it as `data.generation` (`{ label, status, distribution? }`) and `provenance.generation` (new optional field on the envelope schema). `generation` in the input narrows the search to one set. A pool no manager knows is `pool_not_found` naming every manager asked with its label; a chain that answered nothing stays `chain_read_failed`.
...
- **Both MarketCreated shapes in every pool-creation scan.** full-decentralized cork-pools / trading-pairs / the fills join ask for the 7-arg and the 9-arg topic and decode each log with the ABI of its EMITTER's declared wire; rows carry `wire`, `generation` and, on 10-field, the two fees. The scan cache identity carries a row-shape schema (`SCAN_CACHE_SCHEMA` 2), so 0.5.x cursors are ignored. The hybrid existence probe reads each manager through its own `market()` ABI.
- **Emitter roles.** `baseFiller` (the rollover BaseFiller's three-arg `JITMarketCreated`; `rollover.baseFiller` in the config, `classifyAddress` role), `factory` (`RolloverContractDeployed`), `whitelistManager` (the six whitelist events), and the 7-arg `MarketCreated` for 8-field pool managers — a manager is evidence only for its own wire's topic. Every Cork topic in the verified event set now has an emitter role, and a test holds the parity.
- **Read-only generations.** A pre-expiry `cork_prepare_phoenix` action on a pool that lives on a read-only set refuses `generation_read_only`; the three post-expiry settles (withdraw, withdraw-other, redeem) still build.
- SDK (`@cork/core`, root, `/chain`, `/bundle`, `/indexer`): `PoolManagerRef`, `FeeDisagreement`, `feeDisagreementWarnings`, `poolManagerMarket10Abi`, `marketAbiFor`, `MARKET_CREATED_10_TOPIC`, `MARKET_CREATED_TOPICS`, `MarketEmitter`, `BASE_FILLER_JIT_MARKET_CREATED_TOPIC`, `POST_EXPIRY_ACTIONS`. `resolvePoolTokens` takes `{ poolManager, wire }` instead of a bare address; `CorkAddresses` requires `wire`; `decodeMarketRows` takes an optional emitter table; `PoolStateRead.market` is `Market8 | Market10` beside `wire`.

...
- **`cork_query account-state` without `filters.poolId`** returns the account's positions across every generation with a pool manager: the pool list is venue-discovered by default (hybrid: the same `/pools/v1` rows `cork-pools` reads, walked at the venue's 200-row page, each row attributed to the generation its pool manager belongs to; `mode: "full-decentralized"` enumerates from the `MarketCreated` scan instead, decoded per emitter wire; `lite-decentralized` is refused — no RPC-only enumeration is complete, the first live run answered `pools: 0` against 453 known pools), one balance sweep reads cST and cPT per pool over YOUR RPC, and the result keeps the pools with a non-zero balance — `positions[]` (generation, poolId, poolManager, expiryTimestamp, expired, assets, share tokens, balances), `byGeneration[]` (pool count and exact 18-decimal share totals per generation asked), `generations[]`, `scanned`. `generation` narrows the sweep (label or alias; `all` = no narrowing). This result carries no `provenance.generation`; the single-pool read is unchanged. CLI: `ch query account-state --account <a>` prints a positions table.

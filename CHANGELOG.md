...
- Agent evals: five active tasks and one held-out for the order lifecycle (the reserved revision ladder, a `distinct` split across takers, one capacity across two RFQs via `ocoGroup`, what a cancel of one rung retires, the `orders` vocabulary, and the ranked book default; the held-out task hides a `shared` policy decision in plain words), each with an offline fixture test and a self-drive play; the stub builds a real ladder rung fixture through the ladder path itself.

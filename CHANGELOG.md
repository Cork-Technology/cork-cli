...
- **answer-rfq discloses a window sized for a different life than the market's.** The impairment recipe never compares `duration_seconds` with the pool's expiry; a window sized for 7 days on a 30-day market can hit its wall long before expiry, and one sized for 30 on a 7-day market is looser cover than the spread implies. When they differ by more than a day (the slack that absorbs the open→answer clock) answer-rfq says so — `invalid_order_terms`, info; the constraint is still built from the requester's duration, because that IS the pool they derived. Verified from the venue's own source that the block reaches us untouched: the venue "stores and serves this VERBATIM and never interprets it".
- **answer-rfq reads the impairment inline template — `cork-inline-impairment/1`.** The venue types `oracle_params` as a FREE-FORM bag (verified against api-phoenix 0.4.3's openapi), so the schema names are this tool's conventions and no venue change was needed: a requester carries `{schema: "cork-inline-impairment/1", anchor_rate, duration_seconds, apy_spread_percentage (1e18 = 1%), expiry, swap_fee_wad, unwind_swap_fee_wad}` and answer-rfq derives the recipe's 96-byte additionalData from it — three words or NOTHING (a partial block never encodes a zero the requester did not ask for; it warns `invalid_order_terms` naming the missing word(s), and an explicit `jitMarket.additionalData` wins over it). `answer.inline` echoes the schema, both extra words, and `complete`. The liquidity path is unchanged. SDK (root + `/orders`): `INLINE_IMPAIRMENT_SCHEMA`, `INLINE_TEMPLATE_SCHEMAS`, `InlineTemplateParams`/`InlineImpairmentParams`, `inlineAdditionalData`.
- **`argsUints` on recipe-rate-constraint — structured recipe args, no hand-built hex.** The eval added for the impairment recipe exposed a real surface gap: an MCP agent cannot call the SDK's encoders, and hand-padding a 96-byte `args` hex is what it was actually being graded on (two live trials failed exactly there). `argsUints` takes the recipe's additionalData as decimal uint256 words and the tool ABI-encodes them in order (`encodeUintWords`, SDK root + `/registry`); mutually exclusive with `args`. With it, a live agent resolves the impairment constraint in ONE call with correct scales. The venue's own support verified live the same pass: api-phoenix 0.4.3's new `/registry/v1/{chain}/recipes/{address}` serves the impairment recipe's full self-description (the spec re-capture rode a separate commit).
...
- **A gated answer-rfq derive dropped every warning gathered before it.** When derive-cork-pool returned `unavailable`, the answer returned the derive's warnings ALONE — the incomplete-block teaching, `fill_sender_unknown`, the out-of-window expiry note all vanished, and a caller was sent to the recipe's raw revert with no hint that the RFQ's own block was the cause. The reason still rides first; the earlier warnings now follow it. Found by a mutation probe that survived a too-lenient test.

### Changed

- The answer-rfq `jitMarket` description now states what the tool already did: `additionalData` is DERIVED from the RFQ's inline block when omitted, so an RFQ carrying a complete block needs no `jitMarket` at all. Two live agent trials had gone hunting for a payload the tool builds itself (one ran out of turns); with the sentence, both re-trials answered in two calls.
...
- **The probe walk's budget is caller-tunable and operator-defaultable.** `probeBudget` (cork_query, offers only — refused with teaching on every other resource, like `sort` off the orderbook) bounds the walk's eth_calls per side, schema-bounded 1..25; the `CORK_PROBE_BUDGET` environment variable moves the DEFAULT of 6 within the same range (anything outside it is ignored — the range is the contract, an operator typo never turns a read into a chain scan). `data.probing.budget` echoes the effective value. The re-entered orderbook and rfqs legs never see the field.
...
- **Maker readiness on every hybrid book read** (`packages/core/src/handlers/maker-readiness.ts`). One batched read per distinct (maker, makerAsset, sourcing, JIT context) — deduped across rows, issued in the same tick as the liveness and signature reads, so a page still costs one multicall round-trip. The classifier consults the makerAsset's code first: a code-less token with no creating JIT hook is the silent-noop class (`silent-noop`, structural); with a hook but no covering permit, `unborn-cst-no-permit`; with a permit but a CONTRACT maker, `contract-maker-unborn-cst` — the incident. A token with code is judged on allowance (plain or both Permit2 layers, expiry included), the two ERC-2612 escape hatches (both die with a contract maker), balance, and the JIT mint's collateral funding (only ZERO is provable — the mint's cost depends on the fill-time rate). A transport failure is NEVER a verdict: the row reads `unknown` and stays ranked. Every venue-sort row carries `makerReadiness`; a page with proven-not-ready rows warns `maker_not_ready` (info) once.
- **The ranked view excludes what cannot deliver.** A row whose readiness is PROVEN `not-ready` is excluded with `exclusion: "maker-not-ready"` and the chain evidence under `whyNotFillable`. Only the proven verdict excludes — `unknown` and unjudged rows keep ranking. Every reason is fixable by the maker WITHOUT re-signing, and the message says so.
- **taker-fill pre-flights the maker's side.** The fill still builds (build-and-warn: the fix is the maker's, and the bytes may be for later), but a structural gap warns `maker_not_ready`, a plain fund gap warns `would_revert`, and `data.makerReadiness` carries the full verdict. `data.approvals` now also lists the MAKER-side grants (role-tagged), so a maker debugging their own resting order sees what to grant.
- **Probe fills at the top of the book** (`packages/core/src/handlers/fill-simulate.ts`). The offers view and every watch tick that announces a NEW best probe it with the REAL fill calldata (threshold 0, so an auction row probes at its live decayed price) from the caller's fill sender. `fillSimulation` on the row/best: `fillable` (the eth_call succeeds), `maker-ready` (it reverts only pulling the taker asset — TransferFromTakerToMakerFailed — i.e. grant the allowance and it goes through), `would-revert` (named LOP/Cork error or raw selector), `unknown` (transport — never a verdict). Only a row whose maker signature the verifier SETTLED is probed, and a green probe is trustworthy exactly because the readiness gate runs first — the silent-noop class is the one thing simulation cannot see. The top-ranked offer failing its probe warns `would_revert` naming it.
- `maker_not_ready` joins the warning registry and the topic:"warnings" family table.
...
- **A venue row is discovery, not authority (DB-004).** Every path that acts on a venue-sourced signed order now runs the two checks the venue-free inline fill already ran. The first is the extension rule, exactly as OrderLib.isValidExtension decides it at fill: the HAS_EXTENSION flag and the salt binding, all three reverts, not only the hash comparison. The second is the maker signature: ecrecover for an EOA, the maker's own isValidSignature staticcall for a contract maker. Both live in ONE module, `packages/core/src/handlers/order-auth.ts`. On the ranked orderbook, `confirmed` now means the maker signed the row AND its invalidator bit is unspent. Rows carry `makerSignature` (`eoa-verified` | `erc1271-verified` | `unverified`). A refuted signature drops the row under `status_mismatch`. An extension the order never committed to drops the row chain-free under `signature_or_reconstruction_mismatch`. A maker nobody could ask (no RPC, a transport failure, out of budget) leaves the row `unverified`. So `firm`, the `appeared`/`better` announcements, and the ranking now inherit authentication, not only liveness. The venue taker-fill branch refuses a forged or unbound row with no fill bytes, and it replaces the venue's `makerAccountType` claim with the verdict's (echoed on the artifact). refresh-order authenticates the predecessor before it asks for a new signature. An indeterminate outcome (the maker's code read or its ERC-1271 call fails in transport) is `unavailable` + `chain_read_failed`, never a signature verdict. finalize-maker-order applies the same rule.
- **The HTTP deadline is end-to-end (DB-001).** The admission signal was set on a field the venue transport never read. It now rides `VenueDeps.signal` into every venue fetch, composed with the per-call timeout. No venue call starts after the signal fires, and the call in flight is aborted. An abort surfaces as `request_aborted` (a new code in the availability family). It is never `venue_unreachable`, it never feeds the breaker, and it is never retried. The envelope says whether the call never started or was cancelled mid-flight; a mid-flight relay MAY have reached the venue, so the retry keeps the same clientRequestId and the venue's idempotency answers. The caller gets its 504 the moment the deadline elapses, also while the body is still arriving. The admission slot is released when the cancelled work settles, because the slot accounts for work, not for the response.
- **A raw multicall's OUTER target is disclosed (DB-003).** `cork_decode` kind:"calldata" without `to` on Bundler3.multicall bytes now emits `target_unverified` that names the outer target, also when every inner leg verified. A warning-free result had implied an executor check that never happened. Pass `to`, or decode the signed tx, to verify it.
- **create-pool runs on one controller graph (DB-005).** The tool compares the creator's bound CONTROLLER with the configured one, like its registry and pool-manager bindings. A difference is `adapter_binding_mismatch` and builds no bytes. The cST/cPT prediction follows the BOUND controller. Roles and prediction can no longer read two graphs.
- **A transport failure is never an oracle or recipe verdict (DB-007).** A `rate()` read now tells a revert from a transport failure (`oracle.rateReadFailure: "revert" | "transport"` beside `rateReadable:false`). A transport failure is `chain_read_failed` on registry-oracle, derive-cork-pool, recipe-rate-constraint, the JIT ladder, and create-pool's verify pre-flight. It is not `oracle_rate_unreadable`, and it is not `recipe_refused` with advice to change `additionalData`.

### Changed

- **The ocoGroup namespace is disjoint by construction (RC1-NONCE-001).** A clientRequestId may not start with `oco-group:`, the group seed prefix (`OCO_GROUP_NONCE_NAMESPACE` in `@cork/schemas`). The schema refuses such an id with teaching, and `buildMakerOrder` refuses it for SDK callers. A group seed and an id seed are therefore never the same string. No existing nonce changes. The 40-bit truncation that any two seeds share stays the disclosed birthday risk. The wording no longer promises more than that.
- The venue-row fixtures in the offline suite now carry REAL maker signatures. The fake 65-byte signature they used to carry is exactly what the book now refuses.
- SDK (`@cork/core`): `VenueAborted` (root and `/venue`) and `VenueDeps.signal` are new. `OCO_GROUP_NONCE_NAMESPACE` is new in `@cork/schemas`. Fill artifacts echo `makerAccountType` (the verdict's, never the venue's claim). A refusal of an unbound extension carries `data.extensionFault` that names the OrderLib revert. `request_aborted` joins the availability warning family.

### Corrected

- The 0.5.0 notes said that every JIT prepare hands its extraData to the deployed adapter's decoder (RC1-DOC-001). The round-trip runs only with an RPC and an adapter that exposes the helper. An offline build with an explicit constraint, a helper-less adapter, a transport failure, and the deprecated legacy lane all report `data.jit.extraDataLayout: "unchecked: …"`. The output was always honest. The claim was not.
- The 0.5.0 notes said that `RESOURCE_FILTER_KEYS`, `assertFiltersApplicable`, `fetchWithTimeout` and `fetchFollowingSameOrigin` are SDK exports (DB-008). They are not, by design. The filter map and the fetch helpers are internal, and the API-surface gate pins the public surface. The runtime behaviours those sentences describe are unchanged.
- The answer-rfq tool description promised a fill "reserved for the requester" (RC2-DOC-001). Since rc.2 the tool reserves for the requester's LOP caller when that caller is known (`fillSender`, or the RFQ's declared fill_sender). Otherwise it builds OPEN with `fill_sender_unknown`. The description now says so and points at `answer.reach`.

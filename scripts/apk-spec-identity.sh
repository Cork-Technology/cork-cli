#!/bin/sh
# shellcheck disable=SC2016  # single-quoted `${{vars.tag}}` literals below are melange template text, not shell
...
# mise.toml where the Bun version is pinned (default: the repo's; read via toolchain-pin.sh). The spec's `bun` build
...
# reads the pin through the same script, so the two can never disagree about the file.
bun_pin="$(sh "$(dirname "$0")/toolchain-pin.sh" bun "$mise")"

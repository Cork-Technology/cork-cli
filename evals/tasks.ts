...
// the 0.5 line and must not feed 0.6 fixtures). Held-out tasks keep their inline copies untouched
// by rule.
const MAINNET_ADAPTER = (corkDefaults as { generations: Record<string, { primary: string; sets: Record<string, { phoenix?: { corkAdapter?: string } }> }> }).generations["1"]!.sets["mainnet"]!.phoenix!.corkAdapter!;
...
export const TASKS: EvalTask[] = [
  ...
  // ── migration (2026-09-22): the positions read spans every generation (no poolId), and a
  //    fund move's FIRST step is a pool-scoped exit whose provenance names the OLD set. ──
  { id: "migration-positions", prompt: `List every Cork position account ${A} holds on Arbitrum (chain 42161) across ALL contract generations — I do not know the pool ids. Tell me which generation each position is on and which generations you checked.`, expect: { tool: "cork_query", params: { resource: "account-state", chainId: 42161, filters: { account: A } }, state: "ok", answer: /phoenix\/v0\.3-rc\.1[\s\S]*phoenix\/v0\.4-rc\.1|phoenix\/v0\.4-rc\.1[\s\S]*phoenix\/v0\.3-rc\.1/, forbid: ["cork_submit"], maxCalls: 2 } },
  { id: "migration-exit-old-pool", prompt: `I am migrating my funds from my old Cork pool ${MIGRATION_OLD_POOL} to the new pool ${MIGRATION_NEW_POOL} on Arbitrum (chain 42161). Build the unsigned bundle for the FIRST step only: pull 1e18 collateral out of the old pool before expiry, burning at most 2e18 share pairs, owner and receiver ${A}, account ${A}, erc20-approve funding, request id "eval-mig-exit-0001". Do not relay anything. Tell me which contract generation the bundle targets.`, expect: { tool: "cork_prepare_phoenix", params: { chainId: 42161, action: { type: "unwind-deposit", poolId: MIGRATION_OLD_POOL } }, state: "ok", answer: /phoenix\/v0\.3-rc\.1/, forbid: ["cork_submit"], maxCalls: 3 } },
  // ── coverage gaps closed 2026-09-23 (owner: "close the gaps") ──────────────────────────
  // Generation selection on a PREPARE: the alias must reach the tool and the result must name
  // the LABEL. A registry read is the cheapest prepare-free probe of the alias plumbing; the
  // authority op below is the cheapest PREPARE that takes `generation`.
  { id: "generation-previous-prepare", prompt: `Build the unsigned approve transaction that lets the Cork adapter of the PREVIOUS contract generation on Arbitrum (chain 42161) spend my sUSDe 0x211Cc4DD073734dA055fbF44a2b4667d5E5fE5d2 without limit, from account ${A}, request id "eval-gen-prev-0001". Tell me which generation label the artifact was built for.`, expect: { tool: "cork_prepare_phoenix", prelude: ["cork_capabilities", "cork_query"], params: { generation: "previous", action: { type: "authority-onboard" } }, state: "ok", answer: /phoenix\/v0\.3-rc\.1/, maxCalls: 3 } },
  // The connectivity PLEDGE is a first-class choice: a user who says "do not contact the venue"
  // must get full-decentralized on a list read, and the result must say which pledge served it.
  { id: "mode-venue-free-pools", prompt: "List the Cork pools on Arbitrum (chain 42161) WITHOUT contacting the Cork venue API — derive the list from the chain's own events only. Tell me which data mode you used.", expect: { tool: "cork_query", params: { resource: "cork-pools", mode: "full-decentralized" }, state: "ok", answer: /full-decentralized/, maxCalls: 2 } },
  // A CHAINED flow graded as a chain: derive the pool the order would birth, THEN prepare the
  // JIT maker order against it. `require` demands both tools ran; `params` pins the second.
  { id: "chain-derive-then-prepare", prompt: `Two steps on Arbitrum (chain 42161). Step 1: derive the Cork pool a just-in-time market would create for collateral ${SUSDE}, reference ${VBUSDC}, expiry ${JIT_TASK_EXPIRY}, recipe ${LIQUIDITY_RECIPE}. Step 2: prepare an unsigned SELL maker order for that pool — 1e18 of the predicted cST for 1e6 of the reference, account ${A}, request id "eval-chain-0001", one hour expiry — with a jitMarket block naming the same collateral, reference, expiry and recipe. Report the derived pool id.`, expect: { tool: "cork_prepare_orders", prelude: ["cork_capabilities", "cork_query"], params: { action: { type: "maker-order" } }, require: ["cork_query", "cork_prepare_orders"], forbid: ["cork_submit"], state: "ok", answer: /0x[0-9a-f]{64}/i, maxCalls: 5 } },
  // ── the conflict family, one task per verification that the stub can refute ──
  // A finalize with a signature that does NOT recover to the maker: refused, never relayed.
  { id: "conflict-finalize-bad-signature", prompt: `Verify this signature over my prepared Cork maker order and give me the relay artifact (chain 1, request id "${FINALIZE_REQUEST_ID}"). Prepared result: ${JSON.stringify(PREPARED_MAKER_ORDER)}. Signature: ${TAMPERED_FINALIZE_SIGNATURE}. List it as a SELL at 4.1% annualized, no expiry, nonce ${PREPARED_MAKER_ORDER.nonce}, partial fills allowed.`, expect: { tool: "cork_prepare_orders", prelude: ["cork_capabilities"], params: { action: { type: "finalize-maker-order" } }, state: "conflict", code: "signature_or_reconstruction_mismatch", forbid: ["cork_submit"], answer: /(does not|doesn't|did not) (recover|match|verify)|mismatch|invalid signature|not (the )?maker/i, maxCalls: 3 } },
  // A listing whose nonce contradicts the SIGNED makerTraits: the venue would list a lie.
  { id: "conflict-listing-nonce", prompt: `Relay this caller-signed Cork limit order to the venue on mainnet (chain 1), request id "eval-lopsub-0002": ${JSON.stringify(SIGNED_LOP_PAYLOAD)}. List it as a SELL at 4.1% annualized, no expiry, partial fills allowed, and use nonce 7777 for the listing.`, expect: { tool: "cork_submit", prelude: ["cork_capabilities"], params: { action: { type: "lop-order" } }, state: "conflict", code: "listing_traits_mismatch", answer: /nonce|makerTraits|contradict|mismatch|not relayed/i, maxCalls: 3 } },
  // A resting order whose extension names a hook contract nobody here has read: no fill bytes.
  ...
  // ── prepare ────────────────────────────────────────────────────────────
  { id: "prepare-deposit", prompt: `Build me an unsigned bundle that deposits 10 sUSDe (10e18) into mainnet Cork pool ${P} for receiver ${A}, minimum 1 share out, request id "eval-dep-0001". Use erc20-approve funding.`, expect: { tool: "cork_prepare_phoenix", params: { action: { type: "deposit", poolId: P } }, state: "ok", maxCalls: 2 } },
  { id: "prepare-swap", prompt: `Prepare an unsigned Cork swap on mainnet: I want exactly 1e18 collateral out of pool ${P}, receiver ${A}, willing to spend at most 2e18 cST and 2e6 reference. Request id "eval-swap-0001".`, expect: { tool: "cork_prepare_phoenix", params: { action: { type: "swap" } }, state: "ok", maxCalls: 2 } },
  ...
  // Prompts whose expected call REQUIRES chainId (the three prepare_* tools) name the chain: a pool
  // id does not encode its chain, and an agent that asks which chain is right (2026-09-23: it asked
  // on prepare-unwind and ho-cancel after passing on two full runs). Regular prompts say "mainnet";
  // held-out prompts keep their wording and grade the clarifying question via `clarify`.
  { id: "prepare-unwind", prompt: `I hold a locked Cork position in mainnet pool ${P}. Prepare the unwind-swap bundle: 3e18 collateral back in, receiver ${A}, no slippage floors, request id "eval-unw-0001".`, expect: { tool: "cork_prepare_phoenix", prelude: ["cork_capabilities", "cork_query"], params: { action: { type: "unwind-swap" } }, state: "ok", maxCalls: 3 } },
  { id: "prepare-order", prompt: `On mainnet, create the signable 1inch maker order selling 1 sUSDe (${"1000000000000000000"}) for 1 vbUSDC (1000000) on Cork pool ${P}: maker ${A}, sUSDe is 0x9D39A5DE30e57443BfF2A8307A4256c8797A3497, vbUSDC is 0x53E82ABbb12638F09d9e624578ccB666217a765e, request id "eval-ord-0001".`, expect: { tool: "cork_prepare_orders", params: { action: { type: "maker-order", side: "SELL" } }, state: "ok", forbid: ["cork_submit"], maxCalls: 2 } },
  ...
  // Both rollover prompts BIND the signer explicitly ("I sign as …"): the clone address alone
  // left the required top-level `account` unstated, and a strict agent rightly refused to
  // fabricate the roller's address (observed 2026-08-28, zero calls — the miss was the
  // prompt's, not the model's; most agents had been inferring clone-owner = signer).
  { id: "prepare-rollover", prompt: `Build me a signable Cork rollover order on Arbitrum (chain 42161): roll 250e18 srcCST via the ExactSettler 0xF4ffd4b3FAedb784b04d1883119840515f224C2f, I sign as ${A} and my rollover clone is that same address, src pool 0x1111111111111111111111111111111111111111111111111111111111111111, dst pool 0x2222222222222222222222222222222222222222222222222222222222222222, srcCST 0x9D39A5DE30e57443BfF2A8307A4256c8797A3497, dstCST 0x53E82ABbb12638F09d9e624578ccB666217a765e, premium token USDC 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48, min premium per share 0.012e18, open by 1795000000, fill by 1795604800, request id "eval-roll-0001".`, expect: { tool: "cork_prepare_orders", prelude: ["cork_capabilities"], params: { account: A, action: { type: "rollover-intent" } }, state: "ok", maxCalls: 3 } },
  ...
  // ── discovery ──────────────────────────────────────────────────────────
  // Discovery is graded by its OUTCOME: the agent searched the capabilities (any non-empty query —
  // the prompt says "undo a covered position" and never uses the word unwind, so the agent may
  // search in the user's words) and the answer names the unwind variant. 2026-09-23: the literal
  // `search: "unwind"` failed three correct answers whose queries were broader or phrased
  // differently; grading the query wording graded the wrong thing.
    { id: "discover-unwind", prompt: "I'm new to these Cork tools. Which tool and variant do I use to undo a covered position, and what does an invocation look like?", expect: { tool: "cork_capabilities", params: { search: /\S/ }, state: "ok", answer: /unwind/i, maxCalls: 2 } },
  ...
  // "unknown", "no such pool", "not a real pool", "a placeholder". Owner rulings 2026-09-23: grade
  // the meaning; an answer that reports pool STATE still fails (none of these words appear there).
    { id: "ho-nonexistent-pool", heldOut: true, prompt: "Read the live market state of Cork pool 0x1111111111111111111111111111111111111111111111111111111111111111.", expect: { tool: "cork_query", params: { resource: "cork-pool" }, state: "unavailable", code: "pool_not_found", answer: /not exist|not found|no such|not (?:a )?(?:real|valid|known|live|configured) (?:cork )?pool|unknown|placeholder|failed|revert|unavailable/i, maxCalls: 3 } },
  {

...
- Costs real tokens: 10 tasks × ~1–3 turns per labeled run. haiku-4.5 is the default
  grader tier — small models are the most sensitive to schema quality; spot-check with
  sonnet-5 to separate schema defects from model-tier noise.

## Usage

```sh
# current tree
bash evals/live-ab/run.sh new-haiku "$(git rev-parse --show-toplevel)" claude-haiku-4-5

# A/B against an older commit
git worktree add /tmp/ab-before <commit> && (cd /tmp/ab-before && bun install)
bash evals/live-ab/run.sh old-haiku /tmp/ab-before claude-haiku-4-5

python3 evals/live-ab/grade.py old-haiku new-haiku
```

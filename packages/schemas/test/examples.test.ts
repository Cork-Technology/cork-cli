...
describe("example address literals ↔ config.default.json (offline drift gate)", () => {
  ...
  // a dead address — the stub.ts pinned-literal rot class, on the tool surface. The document is
  // the v2 generations file this build reads (every generation's recipe hints count — the flat
  ...
  it("every 0x address in an example that looks like a recipe matches a configured recipe", async () => {
    ...
    for (const chain of Object.values((corkDefaults as { generations: Generations }).generations)) {
      for (const g of Object.values(chain.sets)) for (const addr of Object.values(g.marketRegistry?.recipes ?? {})) configured.add(addr.toLowerCase());
    }

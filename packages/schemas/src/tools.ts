...
export const QueryInput = z.object({
  ...
  probeBudget: z.number().int().min(1).max(25).optional().describe("offers only: hard cap on the eth_calls one probe walk may spend PER SIDE hunting for its 2 maker-side-PROVEN rows (default 6; the CORK_PROBE_BUDGET environment variable moves the default when set to an integer in the same 1..25 range). The cap keeps a book resting many failing orders from turning one read into a chain scan — `data.probing` says when a walk stopped on it"),
...
export const OrdersAction = z.discriminatedUnion("type", [
  ...
  A("answer-rfq", {
    rfqId: z.string().min(1).describe("the open RFQ to answer ('rfq_…') — its record supplies the pair, the notional, the requester, the expiry window, and (if cited) the option's premium and pool expiry"),
    answerId: z.string().min(1).optional().describe("CITED answer: the id of YOUR posted answer on this RFQ (with optionId) — the option's premium_annualized and expiry set the amounts, its market_template the recipe, and the order carries quoteRef so the venue can cross-check the premium. The maker must be the answer's underwriter (cork-api 0.4.1 party rule)"),
    optionId: z.string().min(1).optional().describe("CITED answer: the option on that answer to execute"),
    premiumAnnualized: z.string().min(1).optional().describe("UNCITED answer: the annualized premium as a decimal-fraction STRING (\"0.041\" = 4.1%) when no posted option is cited — this tool never chooses a premium, it computes amounts from yours. Mutually exclusive with answerId/optionId"),
    expiryTimestamp: UnixSeconds.optional().describe("UNCITED answer: the pool expiry the cover runs to (unix seconds; the order's tenor is expiry − now). A cited option supplies its own. Outside the RFQ's expiry_window it is a visible counter-proposal (warned, built)"),
    notionalAssets: TokenAmount.optional().describe("cover size in the COLLATERAL asset's base units; default = the RFQ's notional_assets"),
    collateralAsset: Address.optional().describe("required only when the RFQ lists several acceptable collateral tokens (collateral_asset.one_of) — pick one; an `exact` RFQ needs nothing"),
    reserve: z.boolean().default(true).describe("true (default): reserve the fill for the address that will CALL the LOP — `fillSender` when given, else the RFQ's declared `fill_sender`; when neither exists the order is built OPEN with a `fill_sender_unknown` warning (the requester account may fill through an adapter, and an order reserved for the account would revert PrivateOrder for the only party it was meant for — never guessed). false: an open order any taker may lift"),
  ...
  }).describe("re-rest a resting order of yours before it expires: the SAME terms (assets, amounts, reach, partial-fill flag, Permit2 sourcing, extension — a JIT or auction extension verbatim) with a new expiry and a fresh clientRequestId, on the SAME invalidator nonce — so the old order and the new one share one bit and cannot both fill (the refresh is a revision, not a second exposure). Refused when the maker is not `account`, when the row does not hash to orderHash, or when the chain says the bit is already spent (a refresh of a dead order could never fill — post a maker-order). Returns the same signable maker-order artifact, plus `refreshes`"),
]);
...
export const SubmitAction = z.discriminatedUnion("type", [
  ...
  A("rfq-open", {
    ...
    marketTemplate: z.record(z.string(), z.unknown()).optional().describe("venue market template: either {market_template_id} or {inline:{oracle_recipe, oracle_params}} — oracle_params is REQUIRED on an inline template and is one of two blocks answer-rfq reads (decimal strings; the expiry and fees are part of pool identity; never send {}): cork-inline-liquidity/1 = {schema, anchor_rate, expiry, swap_fee_wad, unwind_swap_fee_wad}, or cork-inline-impairment/1 = the same plus duration_seconds and apy_spread_percentage (1e18 = 1%) for the impairment recipe — the venue types the block as a free-form bag, so the schema name is the contract between requester and underwriter. CONVENTION (2.1.0): put the approved recipe CONTRACT ADDRESS in inline.oracle_recipe — the venue types it as free text (it would happily accept a legacy mode name like 'liquidity'), but the fill path only accepts a registered recipe address, so both sides must put the address here for the quote to be executable"),

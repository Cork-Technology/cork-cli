...
export const OrdersAction = z.discriminatedUnion("type", [
  ...
  A("answer-rfq", {
    ...
    reserve: z.boolean().default(true).describe("true (default): reserve the fill for the requester — the RFQ's fill_sender when it names one, else the requester account (allowedSender low 80 bits; any other LOP caller reverts PrivateOrder). false: an open order any taker may lift"),
    fillSender: Address.optional().describe("reserve for THIS LOP caller instead (the requester's ForSelf ADAPTER when it fills through one — the adapter is the LOP's msg.sender there)"),
  ...
  }).describe("answer an RFQ with a FIRM, reserved cover offer in one call: reads the RFQ (and the cited option), derives the pool the cover creates on fill (derive-cork-pool: recipe → constraint → pool id → predicted cST), computes the amounts exactly as the kernel does — takingAmount = premium × notional × tenor / 365 days in collateral units, rounded toward the maker; makingAmount = notional as 18-decimal cST — reserves the fill for the requester, cites the option (quoteRef), applies the re-rest expiry rule, groups every rung answering the RFQ on one bit (ocoGroup 'rfq:<rfqId>'), and returns the SAME signable maker-order artifact maker-order returns (sign → finalize-maker-order → submit lop-order), plus `answer` with the derivation. Needs an RPC (decimals, derivation) and the venue (the RFQ record). This tool never picks a premium"),
  A("refresh-order", {

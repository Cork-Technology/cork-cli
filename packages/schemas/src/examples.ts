...
export const MATURITY: Record<ToolName, ToolMaturity> = {
  cork_query: {
    ...
    variants: {
      ...
      "trading-pairs": { status: "activated", reason: "centralized (the venue's listed view) or full-decentralized (the pairs that CAN trade, derived from MarketCreated events)" },
      orderbook: { status: "activated", reason: "venue-backed (centralized mode)" },
      fills: { status: "activated", reason: "centralized (venue) or full-decentralized (HyperSync OrderFilled scan, Cork-scoped by the same-transaction share-token join)" },

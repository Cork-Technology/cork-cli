...
// Canonical mainnet USDC — the rollover examples' premium token (the venue rejects a premium
// paid in either cST, so it must be a THIRD asset).
const USDC = "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48";
// rc.2 rollover settlers (v0.1.0-rc.2, identical addresses on 42161 + 8453).
const ROLLOVER_EXACT_SETTLER = "0xF4ffd4b3FAedb784b04d1883119840515f224C2f";

// A real one-leg safeSwap Bundler3 multicall (1 sUSDe out of the demo pool), captured when
// cork_prepare_phoenix still emitted action-only bundles — kept as a DECODER fixture; today's
// prepares always carry the initiator's funding legs around the action.
...
export const TOOL_EXAMPLES: Record<ToolName, readonly ToolExample[]> = {
  cork_query: [
    ...
    { title: "Predict a market before it exists: pool id + cST/cPT + constraint for a pair (Arbitrum; recipe = the approved liquidity recipe CONTRACT of the primary phoenix/v0.4-rc.1 generation; the fees are part of the 10-field id)", input: { resource: "derive-cork-pool", chainId: 42161, filters: { collateralAsset: "0x211Cc4DD073734dA055fbF44a2b4667d5E5fE5d2", referenceAsset: "0xdDb46999F8891663a8F2828d25298f70416d7610", expiry: "1900000000", recipe: "0x679Cbd016587c423f342e5Ba31e58356228c964d", args: "0x0000000000000000000000000000000000000000000000000de0b6b3a7640000", swapFeePercentage: "0", unwindSwapFeePercentage: "0" } } },
    { title: "The approved recipe contracts with their live constants + argument shapes (Arbitrum, the primary generation)", input: { resource: "registry-recipes", chainId: 42161 } },
    { title: "The same read against the PREVIOUS (flat-wire, 0.3.3) generation — `generation` selects the set; every chain-backed input takes it", input: { resource: "registry-recipes", chainId: 42161, generation: "phoenix/v0.3-rc.1" } },
  ...
  cork_compute: [
    ...
    { title: "Ask a recipe what constraint it imposes on a pair — THE step that produces what a JIT order signs (Arbitrum)", input: { chainId: 42161, params: { kind: "recipe-rate-constraint", recipe: "0x679Cbd016587c423f342e5Ba31e58356228c964d", collateralAsset: "0x211Cc4DD073734dA055fbF44a2b4667d5E5fE5d2", referenceAsset: "0xdDb46999F8891663a8F2828d25298f70416d7610", args: "0x0000000000000000000000000000000000000000000000000de0b6b3a7640000" } } },
  ...
  cork_prepare_orders: [
    ...
    { title: "Buy cover as maker at YOUR price + expiry, with just-in-time market creation (Arbitrum): makingAmount/takingAmount set the premium, expirySeconds bounds the order, jitMarket pins the pool identity at signing", input: { chainId: 42161, account: DEMO_ACCOUNT, clientRequestId: "demo-jit-buy-0001", action: { type: "maker-order", poolId: "0x1111111111111111111111111111111111111111111111111111111111111111", side: "BUY", makerAsset: "0x211Cc4DD073734dA055fbF44a2b4667d5E5fE5d2", takerAsset: "0x53E82ABbb12638F09d9e624578ccB666217a765e", makingAmount: "10000000000000000", takingAmount: "50000000000000000", expirySeconds: 86400, jitMarket: { collateralAsset: "0x211Cc4DD073734dA055fbF44a2b4667d5E5fE5d2", referenceAsset: "0xdDb46999F8891663a8F2828d25298f70416d7610", expiryTimestamp: "1790000000", recipe: "0x679Cbd016587c423f342e5Ba31e58356228c964d" } } } },
    { title: "Answer an RFQ with a firm cover offer reserved for its declared fill sender (uncited: your own premium; the RFQ supplies pair, notional, expiry window)", input: { chainId: 42161, account: DEMO_ACCOUNT, clientRequestId: "demo-answer-0001", action: { type: "answer-rfq", rfqId: "rfq_0001", premiumAnnualized: "0.041", expiryTimestamp: "1900000000", jitMarket: { recipe: "0x679Cbd016587c423f342e5Ba31e58356228c964d" } } } },
    { title: "Re-rest a resting order of yours before it expires (same terms, same bit, new 10-minute expiry)", input: { chainId: 1, account: DEMO_ACCOUNT, clientRequestId: "demo-refresh-0001", action: { type: "refresh-order", orderHash: "0x1111111111111111111111111111111111111111111111111111111111111111" } } },
    { title: "A three-rung ladder: two rungs reserved for one taker at improving prices (one bit — the first fill retires the other), plus an open decaying rung on its own bit", input: { chainId: 1, account: DEMO_ACCOUNT, clientRequestId: "demo-ladder-0001", action: { type: "maker-ladder", poolId: DEMO_POOL_ID, side: "SELL", makerAsset: SUSDE, takerAsset: VBUSDC, makingAmount: "1000000000000000000", expirySeconds: 600, rungs: [{ takingAmount: "1000000", allowedSender: "0xc0ffee0000000000000000000000000000000002", label: "reserved-1" }, { takingAmount: "950000", allowedSender: "0xc0ffee0000000000000000000000000000000002", label: "reserved-2" }, { takingAmount: "980000", expirySeconds: 3600, auction: { durationSeconds: 1800, initialRateBump: "500000" }, label: "open-decay" }] } } },
  ...
  cork_prepare_market: [
    ...
    { title: "Unsigned CorkMarketCreator.createNewPool tx — create a JIT pool AHEAD of the fill (the smart-account path around EOA-only ERC-2612 permits: create, cst.approve, fill with no permits)", input: { chainId: 8453, clientRequestId: "demo-market-0003", action: { type: "create-pool", collateralAsset: "0x211Cc4DD073734dA055fbF44a2b4667d5E5fE5d2", referenceAsset: "0xdDb46999F8891663a8F2828d25298f70416d7610", expiryTimestamp: "1790000000", recipe: "0x679Cbd016587c423f342e5Ba31e58356228c964d", constraint: { rateMin: "900000000000000000", rateMax: "1100000000000000000", rateChangePerDayMax: "10000000000000000", rateChangeCapacityMax: "100000000000000000" } } } },
  ],
  cork_track: [
...
export const MATURITY: Record<ToolName, ToolMaturity> = {
  cork_query: {
    ...
    variants: {
      ...
      rfqs: { status: "activated", reason: "hybrid's one unverifiable family (off-chain JSON, no chain footprint): GET /rfqs/v1 discovery feed + single get via filters.rfqId; venue-only in every mode" },
  ...
  cork_prepare_orders: {
    ...
    variants: {
      ...
      "answer-rfq": { status: "activated", reason: "reads the RFQ record and the cited option from the venue, derives the pool (derive-cork-pool), reads the collateral decimals, computes the kernel's amounts (ACT/365, ceil toward the maker; golden-pinned), then re-enters the maker-order path — one artifact, one derivation echo; never chooses a premium" },
      ...
      "rollover-intent": { status: "activated", reason: "offline typed-data build (rc.2 wire: jitMarketHash signed either way); domain-separator parity proven vs all four live settlers (Arbitrum + Base)" },
    },
  },
  cork_prepare_market: {
    status: "activated",

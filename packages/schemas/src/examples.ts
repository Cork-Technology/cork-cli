...
export const MATURITY: Record<ToolName, ToolMaturity> = {
  ...
  cork_prepare_orders: {
    ...
    variants: {
      ...
      "finalize-maker-order": { status: "activated", reason: "pure reconstruction + EIP-712 signature recovery + salt↔extension binding check; emits a content-addressed cork_submit artifact; never signs [K1]" },
      cancel: { status: "activated" },
      "taker-fill": { status: "activated", reason: "bounded venue orderbook lookup + local orderHash re-verification, then canonical uint256-tuple fill calldata (unsigned)" },

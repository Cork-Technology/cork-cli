...
export const REGISTRY = [
  def({
    ...
    phase: 1,
    description:
      "Read any Cork resource (a cork-pool is one expiry of a market — the (collateralAsset, referenceAsset) family; an trading-pair is a LOP pair listing). Live chain state: cork-pool, account-state, pool-whitelist, protocol-config; whitelisted-addresses enumerates whitelist membership from chain events (HyperSync). Venue-backed (centralized): cork-pools, orderbook, fills, trading-pairs, flows (rollover orders/fills/contracts via filters.kind). Use for STATE READS. NOT for derived math (use cork_compute) or building txs (use cork_prepare_*).",

...
//
// Results and errors are rendered by render.ts; this file is only the contract. Both share
// its `wrap`, so the two halves of the human output line up at the same width.
//
...
/** `(required)` in yellow (worth the eye's attention), `(optional)` dimmed — the padding math
 *  runs on the plain word; SGR is applied to the finished token (see render.ts's discipline). */
function reqLabel(isRequired: boolean, s: Style): string {
  return isRequired ? s.yellow("(required)") : s.dim("(optional)");
}

/** Render an object schema's fields as aligned `name  type  (required)` lines + wrapped notes. */
function renderFields(schema: Schema, root: Schema, indent: number, depth: number, out: string[], s: Style): void {
  ...
  for (const name of names) {
    ...
    out.push(`${pad}${s.cyan(name.padEnd(col))}  ${typeLabel(field, root)}  ${reqLabel(required.has(name), s)}`);
    ...
    if (Array.isArray(branches) && depth < 2) {
      renderVariants(branches, root, indent + 4, depth + 1, out, s);
    } else if (isSchema(field.properties)) {
      if (depth < 2) renderFields(field, root, indent + 4, depth + 1, out, s);
...
function renderVariants(branches: unknown[], root: Schema, indent: number, depth: number, out: string[], s: Style): void {
  const pad = " ".repeat(indent);
  branches.forEach((b, i) => {
    ...
    out.push("");
    out.push(`${pad}${GLYPH.item} ${s.cyan(s.bold(heading))}`);
    const note = typeof b.description === "string" ? b.description : undefined;
    if (note) out.push(...wrap(note, indent + 4));
    renderFields(b, root, indent + 4, depth, out, s);
  });
}

/** Full plain-English rendering of a tool contract for the terminal. */
export function formatExplainText(doc: ExplainDoc, s: Style = PLAIN): string {
  const out: string[] = [];
  out.push([s.bold(doc.tool), s.bold(doc.cli), `phase ${doc.phase}`].join(s.dim(`  ${GLYPH.sep}  `)));
  out.push("");
  ...
  if (!props || Object.keys(props).length === 0) {
    out.push(`${s.bold("INPUT")}: none — call with no arguments.`);
  } else {
    out.push(s.bold("INPUT"));
    const required = new Set(Array.isArray(schema.required) ? (schema.required as string[]) : []);
    for (const [name, v] of Object.entries(props)) {
      ...
      if (Array.isArray(branches)) {
        out.push(`  ${s.cyan(name)}  ${reqLabel(required.has(name), s)} — one of these variants:`);
        const note = describe(v, schema);
        if (note) out.push(...wrap(note, 4));
        renderVariants(branches, schema, 4, 1, out, s);
      } else {
        out.push(`  ${s.cyan(name)}  ${typeLabel(v, schema)}  ${reqLabel(required.has(name), s)}`);
        const note = describe(v, schema);
        if (note) out.push(...wrap(note, 4));
        if (isSchema(v.properties)) renderFields(v, schema, 4, 1, out, s);
      }
    }
  }
  out.push("");
  out.push(s.dim("For the machine-readable JSON schema: add --json, or set CORK_EXPLAIN_JSON=1 (CORK_JSON=1 makes every command emit JSON)."));
  return out.join("\n");
}

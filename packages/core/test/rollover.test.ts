...
describe("runTool rollover admission battery (venue-parity gates) + settler generations", () => {
  ...
  it("a RETIRED-generation settler is refused with settler_retired naming the active replacement", async () => {
    for (const [retired, active] of [
      ...
      expect(env.warnings[0]?.message).toContain("arbitrum-v1.1");
  ...
  describe("classifyRolloverSettler over the bundled 42161 record — the pure classification every handler shares", () => {
    ...
    it("retiredSettlerTeaching is ONE string for prepare and submit and carries every active replacement", async () => {
      ...
      expect(text).toContain("PartialSettler of the RETIRED arbitrum-v1.1 rollover generation (retired 2026-08-13)");

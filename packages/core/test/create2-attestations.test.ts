...
// `expected` address from (deployer, salt, initCodeHash) — a tampered entry fails here, and
// (b) agree with the addresses cork-defaults.json actually routes calls to, so the attestation
// layer and the config layer can never drift apart silently. This matters most for the 2.1.0
// registry set: the PREVIOUS generation still answers 2.1.0-shaped calls with garbage, so the
// attestation is the tamper-evidence for the addresses we trust.
...
describe("CREATE2 attestations", () => {
  it("every attestation re-derives to its expected address (local keccak, no chain)", () => {
    for (const a of CREATE2_ATTESTATIONS) {
      const v = verifyCreate2({ deployer: CREATE2_DEPLOYER, salt: a.salt, initCodeHash: a.initCodeHash, expected: a.expected });
      expect(v.match, `${a.name}: derived ${JSON.stringify(v)} != expected ${a.expected}`).toBe(true);
    }
  });

  it("covers the full 2.1.0 registry set and agrees with the config the tool routes calls to", async () => {
    ...
    expect(byName["liquidityRecipe"]).toBe(mr!.recipes!["liquidity"]!.toLowerCase());

    ...
    const env = await runTool(
      "cork_prepare_orders",
      {
        ...
        action: {
          ...
          jitMarket: { collateralAsset: "0x211Cc4DD073734dA055fbF44a2b4667d5E5fE5d2", referenceAsset: "0xdDb46999F8891663a8F2828d25298f70416d7610", expiryTimestamp: "1795000000", recipe: "0xb881DB48ad6DA84a8F0D1cE4150Caf7Ae016Dc55", constraint },
        },
      },
...
describe("decode order on a COMPOSED extension (auction + JIT): BOTH labels", () => {
  it("a taker inspecting a composed venue row sees the auction AND the JIT commitment", async () => {
    ...
    expect(d.fusion).toBeDefined();
    expect(d.jit).toBeDefined();
    expect(String(d.jit!["recipe"]).toLowerCase()).toBe("0xb881db48ad6da84a8f0d1ce4150caf7ae016dc55");

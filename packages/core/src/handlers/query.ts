...
async function handleQueryOffers(input: QueryInput, filters: QueryFilters, chainId: ChainId, ctx: HandlerContext): Promise<Envelope> {
  if (input.mode !== undefined && input.mode !== "hybrid") {
  ...
  const book = await handleQuery({ ...input, resource: "orderbook", sort: "best", filters: { ...(filters.poolId ? { poolId: filters.poolId } : {}), ...(filters.side ? { side: filters.side } : {}), ...(filters.account ? { account: filters.account } : {}) } }, ctx);
  if (book.state !== "ok") return book;
  const rfqs = filters.rfqId
    ? await handleQuery({ ...input, resource: "rfqs", filters: { rfqId: filters.rfqId, view: "current" } }, ctx)
    : await handleQuery({ ...input, resource: "rfqs", filters: { withAnswers: true, view: "current" } }, ctx);

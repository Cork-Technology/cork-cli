...
import { getLopOrderbook, getRfq, parseSignedLopOrder, type SignedLopOrder } from "../datasources/venue.ts";
...
export async function handlePrepareOrders(input: PrepareOrdersInput, ctx: HandlerContext): Promise<Envelope> {
  ...
  if (action.type === "answer-rfq") return handleAnswerRfq(input, action, ctx);
  if (action.type === "refresh-order") return handleRefreshOrder(input, action, ctx);
...
async function handleAnswerRfq(input: PrepareOrdersInput, action: AnswerRfqAction, ctx: HandlerContext): Promise<Envelope> {
  const chainId = input.chainId;
  const lop = LOP_ADDRESSES[chainId];
  if (!lop) return unavailable(chainId, "no_lop", `no known 1inch LOP v4 deployment for chainId ${chainId}`, ctx);
  const nowSecs = nowSecondsOf(ctx);
  ...
  const env = await handlePrepareOrders({ ...input, action: makerAction }, ctx);
...
async function handleRefreshOrder(input: PrepareOrdersInput, action: RefreshOrderAction, ctx: HandlerContext): Promise<Envelope> {
  const chainId = input.chainId;
  const lop = LOP_ADDRESSES[chainId];
  if (!lop) return unavailable(chainId, "no_lop", `no known 1inch LOP v4 deployment for chainId ${chainId}`, ctx);
  const wanted = action.orderHash.toLowerCase();
  ...
  const approvals = await annotateIfExplicitRpc(ctx, chainId, makerApprovalRequirements({ maker: input.account, makerAsset: old.makerAsset, makingAmount: old.makingAmount, lop, usePermit2: traits.usePermit2, orderExpiry: decodeMakerTraits(built.order.makerTraits).expiry }));

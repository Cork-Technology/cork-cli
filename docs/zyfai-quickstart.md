...
- **`activated`** — the whole **read → derive → build → simulate → verify** path: `query` (incl.
  `market-predict`), `compute`, `prepare_market`, `prepare_orders` (maker-order incl. JIT, taker-fill,
  finalize, cancel, rollover-intent), `prepare_phoenix` (all 13 adapter actions), `track`
  (verify / simulate / reconcile), `decode/calldata`. The math ports are checked **bit-exact,
  wei-for-wei** against on-chain reads.
...
- **`specified`** (not built; return `unavailable` by design) — `compute` `dutch-auction-price` and
  `rfq-quote`, `decode` of order/event/receipt, `prepare_phoenix` token-authority ops, and `query`
  `whitelisted-addresses` (needs an indexer). None sit on the pilot's critical path.

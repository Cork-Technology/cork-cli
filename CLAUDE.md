...
| `cork_decode` | Bytes → labeled JSON, all four kinds live: calldata (recursively unwraps Bundler3 multicall), order (LOP v4 hex tuple or JSON fields → makerTraits breakdown + recomputed orderHash; supplied hash/extension cross-checked → `conflict` on mismatch; a Fusion auction extension gets a `fusion` label and a Cork JIT extension gets a `jit` label unpacking what a fill commits to (NOT exclusive: a composed auction+JIT order shows both) — adapter, recipe + carried constraint on 2.1.0, mode string on the legacy generation, permit count — so a taker can inspect a venue row before signing anything), event (one log → named args against the source-verified ABI set; unverified layouts labeled raw), receipt (every log labeled). Reconstructs from bytes; never trusts a supplied parse [K3]. | 1 |

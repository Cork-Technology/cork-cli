...
- CLI: invoked as **`ch`** (launcher `bin/ch`; put `bin/` on PATH). `ch <command> [--json '<input>'] [--rpc-url <url>] [--explain]`. Long form without PATH setup: `bun packages/cli/src/bin.ts <command> …`
  - `--explain` prints a plain-English contract (description + per-parameter breakdown, oneOf variants unfolded) and exits; the machine-readable JSON schema is opt-in via a `--json` value or `CORK_EXPLAIN_JSON=1`. Renderer: `packages/cli/src/explain.ts`.

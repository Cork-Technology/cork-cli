...
| `oracle_not_deployed` | Informational: the recipe's oracle is not deployed yet — on market-predict, market/shares are null (deploy first via `cork_prepare_market`, or sign a JIT order whose fill deploys it); on JIT prepares the pool id assumes the PREDICTED oracle address (a source re-registration before the fill would shift it → OrderNotForPool). |

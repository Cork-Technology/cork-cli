...
| `extra_data_layout_mismatch` | conflict (JIT prepares, maker + taker): the deployed adapter's `decodeExtraData` (the pure decode helper, 0.4.0+) read the extraData this tool encoded DIFFERENTLY — `data.differing` names the fields; no bytes. An adapter without the helper is `data.jit.extraDataLayout: "unchecked: …"`, agreement is `"verified-on-chain: …"`. Encoder ↔ EVM parity: `experiments/fork-harness/test/JitExtraDataDecoder.t.sol` on a fixture `jit-extra-data-fixture.test.ts` drift-gates. |

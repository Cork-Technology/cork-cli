...
| `cork_submit` | The **only** side-effecting tool: relays caller-signed/authored payloads to the venue — `rollover-order`, `lop-order`, `rfq-open`, `rfq-answer` (REVISIONS — newest per underwriter wins; optional `supersedes`), `rfq-counter` (requester's non-committal counter-bid; fraction-string premium < 0.5, string-decided; `optionRef` pre-flighted) — all off-chain POSTs. Commitments recomputed before relay [K3]; never signs [K1]. | 3 |

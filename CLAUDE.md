...
declared wire it does not know (`phase_gated`) rather than guessing. Labels are Distribution names
where one exists, ours for the eras before it. Per chain:
...
= 176 arbitrum-v1.1 + 3 arbitrum-legacy + 274 phoenix/v0.3-rc.1; 466 on Base, all phoenix/v0.3-rc.1;
...
`commit` as source; `topic:"verify"` re-derives them — the `mainnet` and `phoenix/v0.3-rc.1`
sets are attested; the `phoenix/v0.4-rc.1` set has NO attestation because the Distribution
...
(phoenix/v0.3-rc.1). WHY it cannot simply move: a schema-1 file whose primary moved would send

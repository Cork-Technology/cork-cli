...
| `cork_prepare_market` | Unsigned market-infrastructure txs: deploy-oracle = MarketRegistry.deploy(ca, ref, mode) (price\|nav, default price), deploy-fixed-oracle = deployFixedRateOracle(rate) (CREATE2-salted on the RATE — the oracle a FIXED order's rateOverride produces), and **create-pool** = CorkMarketCreator.createNewPool(params) (cork-periphery 0.1.0, same address both chains, verified on-chain 2026-08-28 incl. a live Base eth_call returning the tool's exact predicted (poolId, cst, cpt)): the pool a JIT order derives, created AHEAD of the fill — THE smart-account path around EOA-only ERC-2612 JIT permits (batch create-pool → cst.approve(LOP) → permit-free fill, enableJitMint false). create-pool mirrors the jitMarket wire fields (minus mint flag/permits), auto-resolves the constraint via recipe.resolve (or takes it explicitly for OFFLINE byte-building — the creator resolves the oracle on-chain, no oracle address needed in the calldata), and pre-flights bindings/roles/coherence/verify/existence/maxExpiryDuration/zero-rate with the derived poolId + predicted shares in `data`. All permissionless + idempotent; 42161 + 8453. Markets are otherwise created JIT by LOP fills (maker-order + `jitMarket`). Results carry `data.execution`. | 4 |

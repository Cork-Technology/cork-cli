...
- CLI: invoked as **`ch`** (launcher `bin/ch`; put `bin/` on PATH). Long form without PATH setup:
  ...
  - **Input**, three interchangeable forms: `--json '<object>'` (canonical wire shape, same as MCP) ·
    ...
    `--mode` stays the data-backend selector), and the resource accepts the singular `rfq`
...
JIT/derive path work — but **no pools exist on the v1.3 pool manager yet** (market reads
`chain_read_failed` until markets are created there), the adapter's POOL_CREATOR + FEE_MANAGER

...
| `cork_prepare_market` | Unsigned oracle-infrastructure txs against the 2.1.0 registry: deploy-wrapper = MarketRegistry.deploy(ca, ref, mode) (mode-keyed price|nav, default price) and deploy-fixed-oracle = deployFixedRateOracle(rate) (keyed on the RATE, CREATE2-salted — the oracle a FIXED order's rateOverride produces). Both permissionless + idempotent; Arbitrum. Both results carry `data.execution`. Markets themselves are created JIT by LOP fills — `cork_prepare_orders` maker-order + `jitMarket`. | 4 |

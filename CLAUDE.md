...
- CLI: invoked as **`ch`** (launcher `bin/ch`; put `bin/` on PATH). Long form:
  ...
  - `ch query orderbook --watch [--interval <s>] [--iterations <n>]` loops the ranked read,
    threading each `watermark` into the next `since`; prints the first read, then only ticks that
    changed. Owns since/wait (refuses them alongside); `--since`/`--wait` alone are one-shot flags.
    `ch query rfqs --watch` (2026-09-11) is the RFQ-FIRMNESS liveness loop: each tick re-reads the
    feed WITH answers (forces `withAnswers` on a list read; a `rfqId` read embeds them already),
    reduces every RFQ to {version, firmQuotes, indicativeQuotes, counter, backed}
    (`packages/cli/src/watch-rfqs.ts`, pure), and prints the ticks where an RFQ appeared/went,
    its `version` moved, or the alert set moved; `data.changes.unbacked` names every requester
    counter that ACCEPTS a quoted option (`counter.counter.option_ref`, the venue's live shape)
    which no live resting order cites — the dead-kernel-underwriter signature of 2026-09-10 (an
    accepted quote nobody rested); `backedNow` when a citing order later rests. A cited option
    beyond the truncated embed is refuted only when NOTHING on the RFQ is firm, otherwise
    `backed: null`. The diff is CLI-side (the venue has no change feed beyond `version`); each
    tick costs the rfqs read plus its ranked-book join. Mutation-probed (`watch-rfqs-*`,
    `cli-watch-rfqs-*`).
...
| Tool | Use when | Phase |
|---|---|---|
| `cork_capabilities` | Discover/introspect: list tools, `search` by keyword, `topic` for docs, `topic:"verify"` re-derives deployed addresses via CREATE2. Doc topics resolve before tool names, by name OR alias: `topic:"signing"` (`execute`/`broadcast`/`sign-and-broadcast`) is the sign-and-broadcast guide; `topic:"warnings"` (`codes`/`envelope`/`states`) is the envelope contract + the warning-code FAMILY table, generated from `WARNING_FAMILIES` (packages/schemas) — the machine-readable registry a test holds to exact set-equality with every code the handlers emit (packages/core/test/warning-registry.test.ts), so an undocumented code or dead entry fails offline; the per-code table below stays the repo-internal working reference. `topic:"units"` (`scales`/`decimals`/`wad`/`fixed-point`) is the scale table — ten conventions, unit OWNER per row, a "5% is written" exemplar, the three collisions (1e18=1.0 vs 1e18=1%; `premium`'s four meanings; rateMin/rateMax absolute vs legacy bands), in two-axis Reserve/ToB notation (`D18{1}` vs `D18{%}`) beside each field's own description wording; the premium scale tripwires route to it. `topic:"orders"` (`order-lifecycle`/`reservation`/`oco`/`one-cancels-the-other`/`ladder`/`liveness`/`exclusivity`) is the ORDER vocabulary — one term per concept for reach (open | reserved for a FILL SENDER), fill regime (single-fill bit invalidator; partial fills still spend the bit), groups (shared nonce = one-cancels-the-other; a ladder is a group with a purpose; dead-by-sibling is invisible to the venue), series/epoch, price shape (fixed | decaying), provenance (cited | uncited; firm = a live cited order), the liveness table (who knows first), and a synonyms table (dedicated/private/single-taker → reserved; OCO/OCA → group); a parity test holds its field rules to the schema descriptions and its reach values to `BOOK_EXCLUSIVITY`; `private_order` routes to it. `topic:"generations"` (`generation`/`wires`/`primary`, 2026-09-22) is the GENERATION model — what a generation is, labels + statuses, how a prepare picks one (`generation` input, primary by default), how a pool read resolves one from the chain, the wire table per block, which sets are attested. One constant (`DOC_TOPICS`, packages/schemas) feeds the MCP `instructions` string and every `/docs/<topic-or-alias>` page via `findDocTopic`; a parity test fails if the table and a field's description disagree. No-args also returns the doc-topic CATALOG (`docTopics`: name/aliases/summary per topic). Start here when unsure. | 1 |
| `cork_query` | **State reads** (taxonomy: a cork-pool is ONE EXPIRY of a market — the pool family over one (collateralAsset, referenceAsset) pair; a trading-pair is a tradable LOP listing). Every chain-backed input takes an optional `generation` label — or an ALIAS: `primary` (= omitted) / `previous` (the newest ACTIVE non-primary set carrying the block kinds THIS call needs; `resolveGenerationAlias`, one resolver, results carry the LABEL never the alias; `all` refused with teaching — the multi-generation read is account-state without poolId) (0.6, 2026-09-22 — see "Address config"): a registry-scoped read (registry-*, derive-cork-pool) follows the SELECTED generation (primary when omitted); a POOL-scoped read (cork-pool, account-state, pool-whitelist) follows the POOL's generation, resolved from the chain by ONE batched `shares(poolId)` read across every configured pool manager (`resolvePoolGeneration`, generations.ts; `generation` narrows the search to one set) — every result carries `data.generation` `{ label, status, distribution? }` + `provenance.generation`, a pool no manager knows is `pool_not_found` (unavailable) naming each manager asked with its label, and a manager that THROWS keeps `chain_read_failed`. protocol-config reports the selected generation and the chain's full list (every block's addresses + wire). Live chain: cork-pool (on a 10-field manager the fees decode FROM the `market()` tuple — the identity — and are compared with the swapFee/unwindSwapFee views, a disagreement = `invalid_state` naming both; `data.wire`), account-state (with `filters.account` ALONE = the account's cST/cPT POSITIONS across every generation with a pool manager — `handlers/query-positions.ts`: pools enumerated under the mode's pledge — the MarketCreated scan over YOUR RPC by default (lite; `windowedRpcSource` asks for the WHOLE remaining range first and shrinks on refusal — live 2026-09-23: 455 pools on Arbitrum / 470 on Base, complete in ~3 s, MORE than the venue lists; the fixed 50k window it replaced walked 1M blocks from block 0 and found nothing), HyperSync under `full-decentralized` (windowed fallback labeled when no token), the venue's `/pools/v1` list under `hybrid` (the caller's `pageSize`/`maxPages` honoured like cork-pools, `expiry` through `venueInstant` (`@cork/schemas`, THE boundary parser for every venue timestamp: integer unix seconds — every RFQ field's live shape — or strict explicit-zone ISO-8601 — `/pools/v1`'s; canonical pair `YYYY-MM-DDTHH:MM:SSZ` + seconds; zone-less, date-only, natural-language, fractional, and past-2100 values refused; answer-rfq `valid_until`/option expiry and `rfqs --watch` read through it too); every position row carries `expiry` (ISO) beside `expiryTimestamp` (seconds), one spelling for both sources) — the opt-in for an endpoint capping eth_getLogs at 1,000 blocks — an endpoint refusing even the floor throws `LogRangeCapError`: an AUTOMATIC endpoint fails over once (breaker + re-resolve, `rpc_fallback` names both hosts), an EXPLICIT `--rpc-url` fails loudly as `chain_read_failed` naming the host (owner ruling 2026-09-23); balances swept over YOUR RPC (the FIRST client — the cap is about eth_getLogs, not eth_call); zero-position pools dropped; `generation` label or alias narrows; no `provenance.generation`, each row carries its own; topic:"migration"), pool-whitelist, protocol-config, the registry-assets/-oracle/-recipes/-denominations/-feeds 2.1.0 views (see MarketRegistry section; oracles mode-keyed price\|nav per pair PLUS fixed-rate keyed on `filters.rate`), derive-cork-pool (derive ONE pool before it exists — recipe + off-chain constraint via recipe.resolve, LOCAL pool id, cST/cPT via state-override simulation, existence; needs `filters.collateralAsset+referenceAsset+expiry+recipe`; optional `oracleSalt` (default zero — the salt of the pair's FIRST oracle wrapper on a nested-wire generation, refused non-zero on flat) and `swapFeePercentage`/`unwindSwapFeePercentage` (default 0, 1e18 = 1% — PART OF THE POOL ID on a 10-field generation); the derivation a JIT fill runs; identity PINNED once an order carrying the constraint is signed). Registry reads follow the selected generation's `marketRegistry.wire`: on `nested` (0.5.0) denominations are address units (`{ unit, symbol, name }`, `filters.address`; `filters.label` refuses with teaching) and feeds carry no `feedDecimals`; on `flat` (0.3.3) the label-keyed table and `feedDecimals` stay. Venue-discovered + chain-verified (hybrid, the default for lists; renamed from `centralized` 2026-08-13 — rows carry `verification:'confirmed'|'unverified'`, chain-refuted rows DROP with `status_mismatch` [K7], budget 50 rows/page, no-RPC → all rows labeled unverified; trading-pairs never drop — chain existence is an `exists` annotation; ask `cork_capabilities topic:"modes"`): cork-pools, orderbook, fills, trading-pairs, flows (`filters.kind`), rfqs (default `state=open`; `filters.rfqId`; `withAnswers`; `filters.view` 'current' = the negotiation frontier — one current answer per underwriter + the requester's counter; monotonic `version` = cheap change-polling; `filters.excludeRequestPrefix` drops `request_id` prefixes server-side, e.g. 'healthcheck-'; `filters.underwriter` = only RFQs that underwriter has ANSWERED, server-side (venue 0.4.1, 2026-09-04 — the answering side's mirror of `account`); rows read WITH answers (`withAnswers` or `rfqId`) carry `firm` on every answer and option from the same ranked-book join `offers` makes — a LIVE resting row cites it via quoteRef — plus `firmQuotes`/`indicativeQuotes` per RFQ and a `firmness` block; the book leg failing serves the rows without flags and says so). offers (2026-09-02) = the unified discovery view: the ranked orderbook joined on `quoteRef` with the RFQ answer options it cites (`provenance` cited|cited-unresolved|uncited, `quote`), plus `indicative` = served options NO live order backs (counted, never ranked; a quote is FIRM only when a live order cites it); composed by re-entering the handler for orderbook + rfqs, hybrid only, `filters.rfqId` scopes to one request; with `filters.account` + an RPC, each side is PROBE-FILLED from the top (real fill calldata, threshold 0) until 2 rows prove maker-side deliverable (`fillSimulation` fillable|maker-ready — maker-ready because a sender without the taker allowance reads it on every healthy row) or the walk's budget runs out (`probeBudget` input 1..25, default 6, `CORK_PROBE_BUDGET` moves the default; `data.probing` per side; failing probed rows warn `would_revert`; the watch tick still probes only its announced best). orderbook is RANKED by default (`sort:"best"`, 2026-09-02): fillable rows for `filters.account` by unit price from the SIGNED amounts (a decaying row at its price now), reserved-for-account wins ties, confirmed beats unverified, longer-lived beats shorter; OCO rungs (same maker+nonce) collapse to their best with `group.collapsed`; the rest ride under `excluded` with `whyNotFillable`; `sort:"venue"` = the venue's newest-first rows verbatim; `sort` on another resource is refused (`packages/core/src/orders-rank.ts`, mutation-probed). Rows carry `allowedSender` + `exclusivity` decoded LOCALLY from the signed makerTraits (chain-free; `filters.account` = the fill sender to classify against — the ForSelf ADAPTER on a wrapper fill) and `makerSignature` (`eoa-verified` chain-free by ecrecover, `erc1271-verified` by the maker's own isValidSignature staticcall, `unverified` when nobody could ask a non-recovering maker — and bytes ecrecover cannot read, the 85-byte Safe7579 `validator ++ sig` included, are NEVER dropped chain-free: rc.4 dropped them as unparseable, rc.5 routes them to the code probe / ERC-1271 leg; the ONLY chain-free drops are the hash lie and the extension lie, `status_mismatch` counts the chain's drops alone, `verification.dropped` the total); since 2026-09-11 (audit DB-004) a venue row is DISCOVERY, not authority: `confirmed` means the maker signed it AND its bit is unspent, rows whose signature is refuted (an EOA that did not sign; a contract maker whose isValidSignature rejects) drop under `status_mismatch`, rows whose extension bytes are not the ones the salt/makerTraits commit to (OrderLib.isValidExtension, all three reverts) drop chain-free under `signature_or_reconstruction_mismatch`, and the venue taker-fill branch + refresh-order run the SAME extension rule and signature ladder as the inline path (`packages/core/src/handlers/order-auth.ts`, the ONE home of both checks); rows that do not hash to their claimed orderHash drop chain-free; a row that fails shape-parse serves venue-claimed, labeled unverified, WITHOUT exclusivity (absence of `exclusivity` = not locally decoded). WATCH (2026-09-02): every ranked orderbook read returns `watermark` (opaque, client-side — the venue has no `updated_after`: the live set incl. collapsed rungs + best per side, per fill sender); `since` = a prior watermark → `changes` {appeared (confirmed only), gone, unconfirmed, best{changed,died} per side, better = confirmed rows better than the watermark's best: lower unit price on SELL / higher on BUY, or same price reserved-for-account vs open}; `wait` 1..25 s long-polls at a 2 s cadence, poll-COUNT driven (`ctx.sleep` injectable), `waited` says how it ended; refused off the orderbook, under sort venue, without since, on a sender mismatch (`packages/core/src/orders-watch.ts`, mutation-probed `watch-*`/`query-wait-*`/`cli-watch-*`). Event-derived: whitelisted-addresses (replayed from WhitelistManager events over HyperSync, needs ENVIO token; `filters.poolId` scopes); cork-pools/trading-pairs/fills/flows(kind=fills\|contracts) also serve `full-decentralized` (HyperSync) with a live RPC tail merged past the archive head (`data.liveTail`): trading-pairs = the pairs that CAN trade (one row per created pool; the venue's listing metadata is off-chain, said in `data.note`); pool-creation scans ask BOTH `MarketCreated` topics (7-arg on 8-field managers, 9-arg on 10-field) and decode each log with the ABI of its EMITTER's declared wire — never by topic guessing (rows carry `wire`, `generation`, the two fees on 10-field; a log of the other wire from a manager is `emitter_role_mismatch`; `SCAN_CACHE_SCHEMA` 2 discards 0.5.x cursors — a 7-arg-only cursor on a 10-field manager would stay empty forever), fills = CORK-SCOPED by a same-transaction share-token join (rows carry `poolIds`; `filters.poolId` scopes the join; a chain with no pools answers an honestly empty feed). Venue lists are **bounded traversals** (`data.pagination`; partial = `ok`+`pagination_incomplete`, repeated cursor = `conflict`; `cursor`/`pageSize`/`maxPages` control it). Every venue list — rollover included since venue 0.3.5 — paginates on the same opaque keyset cursor (a decimal cursor from an offset-era rollover result is still accepted by the venue for one request and upgraded; since venue 0.3.16 its offset-deprecation notice fires only on ACTUAL legacy pagination, so compliant reads carry no `venue_notice`). rollover contracts take `filters.factory` (one wallet owns one clone PER factory generation) and orders/fills take `filters.settler`; full-decentralized rollover fills/contracts scan ACTIVE + LEGACY generations from the earliest seed block (rows disclose the emitting settler/factory), and a factory/settler filter SCOPES the scan to that address and ITS generation's seed (one mechanism, `generationScanTargets` in config-remote). | 1 |
...
| `cork_decode` | Bytes → labeled JSON, five kinds: calldata (a Bundler3 multicall unwrapped recursively, OR one recognized single call — Cork adapter action, ERC-20 leg, ForSelf call, a market-infrastructure call (registry deploy/deployFixedRateOracle, CorkMarketCreator.createNewPool → kind "market", role-verified against the configured registry/creator), or a 1inch LOP v4 fill/cancel, which becomes a `lop` leg carrying the order, amount, decoded taker traits, the `_parseArgs` split of receiver/extension/interaction, and — once the chain is known — the EIP-712 `orderHash`, maker-traits breakdown, and the same `jit`/`fusion` labels as kind order; a malformed body degrades to an UNREADABLE leg naming the function, never a throw); **tx** (a SIGNED raw transaction — the validate-before-broadcast step: recovered signer, to/value/chainId/nonce/gas, target named against known Cork deployments, inner legs + summary); order (LOP v4 hex tuple or JSON → makerTraits breakdown + recomputed orderHash; supplied hash/extension cross-checked → `conflict`; a Fusion extension gets a `fusion` label, a Cork JIT extension a `jit` label unpacking adapter/recipe/carried constraint/permit count; both can appear on a composed order); event (one log → named args against the source-verified ABI set; unverified layouts labeled raw); receipt (every log labeled). Reconstructs from bytes; never trusts a supplied parse [K3]. | 1 |
...
| `cork_prepare_orders` | Build **unsigned** signable artifacts (every chain-backed input takes `generation`; a jitMarket block follows the selected generation's `marketRegistry.wire` — `nested` on the cork/v0.4 primary: `extraData` + `oracleSalt` (default zero) inside MarketParams, a 10-field pool id with the fees inside; `flat` on cork/v0.3, byte-identical to 0.5.1; a non-zero `oracleSalt` on a flat/legacy generation refuses naming the generation; a declared wire this build does not implement refuses `phase_gated`; rollover-intent takes its `jitMarket` wire from the SETTLER's generation — `0.2` inserts `oracleSalt` into the JITMarketParams typehash, `rc.2` stays byte-identical, `rc.1` refuses a jitMarket): maker-order (incl. extension/JIT orders, `ocoGroup` — a shared invalidator nonce = one-cancels-the-other — and `auction` — a Cork-native DECAYING-PREMIUM order using the deployed Fusion settlement as a pure amount getter: no postInteraction, fills stay permissionless, the signed takingAmount is the floor; composes with jitMarket in one salt-bound extension) / **maker-ladder** (2–32 rungs in one call, a FAN-OUT over the maker-order path with rung ids `<ladderId>:<index>`, `noncePolicy` shared-reserved | shared | distinct, `capacity`, one collapsed notice, fail-closed on any rung; the `auction`/`jitMarket` blocks are shared schemas `MakerAuctionWire`/`MakerJitMarketWire`) / cancel (result carries `retires`); **finalize-maker-order** (reconstruct exact bytes, verify the external signature — EOA ecrecover or the ERC-1271 staticcall the fill performs — emit a verbatim `cork_submit` artifact with `makerAccountType`; never signs); **taker-fill** (fetch + re-hash a resting venue order — or take it inline via `signedOrder`, the VENUE-FREE path: the exact shape finalize's submitInput carries, re-hashed against `orderHash`, salt↔extension binding checked, maker signature verified via the shared ladder (EOA ecrecover / the ERC-1271 staticcall), venue never contacted — emit canonical uint256-tuple fill calldata, unsigned; an AUCTION row is auto-detected — the default cap becomes the curve's CEILING (a floor cap would revert mid-decay), current/ceiling/floor in `data.auction`; `interaction` packs a TAKER interaction — `adapter ++ extraData`, length at takerTraits bits 200-223 — how an underwriter lifts a BUY-cover order (JIT cST mint mid-fill); `forSelf: { adapter, poolId }` emits fillOrderForSelf — target forced to the caller, taker interactions impossible, allowance to the ADAPTER — with a liveness pre-flight refusing dead rows [K7]); and the rollover ERC-7683 OrderData (CorkSettler domain, intent hash recomputed). **answer-rfq** (2026-09-02): the underwriter's every-RFQ sequence as ONE call — reads the RFQ record (+ the cited option; own-answer party rule enforced), reads the requester's INLINE TEMPLATE block `market_template.inline.oracle_params` under the `cork-inline-liquidity/1` contract — or, since 2026-09-21, `cork-inline-impairment/1` (the same block + `duration_seconds` + `apy_spread_percentage` 1e18=1%, encoded as the impairment recipe's three words or NOTHING: a partial block warns naming the missing word, never encodes a zero; `duration_seconds` more than a day off the answer's tenor is disclosed; the venue stores the bag VERBATIM, so the schema name is the requester↔underwriter contract) (rc.5; the cited option's block wins; `inlineParamsOfTemplate`/`encodeAnchorArgs`/`INLINE_LIQUIDITY_SCHEMA` in `orders-answer.ts`, SDK `/orders`): the anchor rides as `extraData` = abi.encode(anchorRate) (0.6: the market-registry 0.5.0 word on every jitMarket input; `additionalData` is the accepted alias + `deprecation_notice`, both present and different refuse), the fees fill the JIT block, an expiry that differs from the answer's is `invalid_order_terms` (info — a different pool), and the constraint the derivation resolved is PINNED into the order so derive and ladder cannot tick apart on a NAV pair (the ladder still runs recipe.verify and every binding check on an explicit constraint); `answer.inline` echoes it with `anchorHonored`. VERIFIED LIVE 2026-09-11 on the 42161 NAV wrapper: LiquidityRecipe.resolve anchors on the LIVE oracle rate and IGNORES args while the oracle is deployed, and honours abi.encode(anchorRate) only while it is undeployed — so on a live pair the requester's anchor cannot pin the pool, and `rate_drift_notice` (info) names the anchor, the live rate and the pool the order actually births; explicit jitMarket fields win over the inline block. Then derives the pool via derive-cork-pool, reads collateral decimals, computes the kernel's amounts (`premiumAmount`: ACT/365, ceil toward the maker, golden 4931507 pinned; making = notional as 18-dec cST), reserves for `fillSender` or the RFQ's declared `fill_sender`, else OPEN + `fill_sender_unknown` (never a guessed reservation — the requester may fill through an adapter; 2026-09-04), expiry = re-rest rule max(90 s, min(600 s, remaining/2)), `ocoGroup` default `rfq:<rfqId>`, then RE-ENTERS the maker-order path (same artifact + `answer` echo; `data.execution` adds the refresh step). **refresh-order**: venue lookup + re-hash + invalidator read (spent bit → `status_mismatch` conflict), same terms rebuilt on the SAME nonce (`MakerOrderArgs.nonce` pin) with a new expiry; `refreshes` in data. U8: a CONTRACT maker with a missing JIT pool gets `contract_maker_pre_rest` + a create-pool-first execution list. Lifting an offer is deliberately NOT a sugar (offers → taker-fill orderHash). All but finalize-maker-order carry `data.execution`. maker-order, finalize-maker-order, and taker-fill (raw + forSelf) also carry **`data.approvals`** — the order-lifecycle token grants (holder/token/spender/stage/mechanism + the UNSIGNED approve tx per entry [K1]; Permit2 sourcing = BOTH layers, JIT = embedded-permit coverage marked EOA-only + the adapter's collateral pull; confirmed-missing → `approval_missing`). | 3 |
...
| `cork_prepare_market` | Unsigned market-infrastructure txs, built for the selected `generation` (primary when omitted) on ITS registry wire: deploy-oracle = MarketRegistry.deploy(ca, ref, mode[, oracleSalt]) (price\|nav, default price; the nested 0.5.0 registry takes the salt, the flat one does not), deploy-fixed-oracle = deployFixedRateOracle(rate) (CREATE2-salted on the RATE — the oracle a FIXED order's rateOverride produces), and **create-pool** = CorkMarketCreator.createNewPool(params) (nested: the 0.5.0 creator IN the registry package, 10-field MarketParams with `extraData` + `oracleSalt` + the two fees as identity; flat: cork-periphery 0.1.0's 9-field struct, same address both chains, verified on-chain 2026-08-28 incl. a live Base eth_call returning the tool's exact predicted (poolId, cst, cpt)): the pool a JIT order derives, created AHEAD of the fill — THE smart-account path around EOA-only ERC-2612 JIT permits (batch create-pool → cst.approve(LOP) → permit-free fill, enableJitMint false). create-pool mirrors the jitMarket wire fields (minus mint flag/permits), auto-resolves the constraint via recipe.resolve (or takes it explicitly for OFFLINE byte-building — the creator resolves the oracle on-chain, no oracle address needed in the calldata), and pre-flights bindings/roles/coherence/verify/existence/maxExpiryDuration/zero-rate with the derived poolId + predicted shares in `data`. All permissionless + idempotent; 42161 + 8453. Markets are otherwise created JIT by LOP fills (maker-order + `jitMarket`). Results carry `data.execution`. | 4 |
...
| `chain_read_failed` | RPC answered but the read reverted (or every pool manager THREW during pool-generation resolution — the original error is kept so the breaker still learns). Since 0.6 a pool that is simply absent is `pool_not_found`, not this. |
| `pool_not_found` | unavailable (2026-09-22): every configured pool manager's `shares(poolId)` answered zero — the pool exists on NO generation of that chain; the message names each manager asked with its generation label. Pool-scoped reads, the three chain compute kinds, track marketRef, the 13 phoenix actions and the ForSelf twins all resolve this way (`getPoolDep` → `resolvePoolGeneration`); no funding legs are built. |
| `generation_unknown` / `generation_read_only` | The `generation` gate (0.6): the label names no configured set on this chain — invalid input listing the chain's labels / the named set (or the set a poolId resolved to) is `read-only` — reads, decode, attribution and classification serve, a PRE-expiry prepare refuses (the three post-expiry settles withdraw/withdraw-other/redeem still build; `POST_EXPIRY_ACTIONS` is the one set that also drives the expiry preflight). |
...
| `target_unverified` | Info on ok decode: ONE warning naming every labeled leg nobody could vouch for — an ERC-20 token call, an integrator-deployed ForSelf adapter, a chain with no configured contract for the role, or raw `kind:"calldata"` for a single call with no claimed target (decode the signed tx, or pass `to` — the address you intend to send to — to run the same verification now; a claimed `to` that contradicts the configured contract is a conflict, incl. a multicall claimed at a non-Bundler3 address; `to` on any other kind refuses — a signed tx carries its own). Honest "could not check", never a contradiction. Every leg carries `verification: trusted\|mismatch\|unverified`. |
...
| `deprecation_notice` | Info on ok: still-supported sugar used (e.g. `mode` → recipe address; `additionalData` → `extraData` on a jitMarket/create-pool input, the 0.5.0 word); message teaches the new shape. |
| `extra_data_layout_mismatch` | conflict (JIT prepares, maker + taker): the deployed adapter's `decodeExtraData` (the pure decode helper, 0.4.0+) read the extraData this tool encoded DIFFERENTLY — `data.differing` names the fields; no bytes. An adapter without the helper is `data.jit.extraDataLayout: "unchecked: …"`, agreement is `"verified-on-chain: …"`. The round-trip runs only when an RPC resolved and the adapter exposes the helper: an offline build with an explicit constraint, a helper-less adapter, a transport failure, and the deprecated legacy lane all report `unchecked`, never a verification that did not happen (the 0.5.0 changelog's "every JIT prepare" was the aspiration, not the coverage — audit RC1-DOC-001). Encoder ↔ EVM parity: `experiments/fork-harness/test/JitExtraDataDecoder.t.sol` (flat AND nested decoders since 0.6; `fixtures/jit-extra-data-nested.json`) on fixtures `jit-extra-data-fixture.test.ts` drift-gates. |
| `implementation_gate_bypassed` | Info on ok JIT prepares built under `CORK_ALLOW_UNAPPROVED_CODE=1`: the adapter's off-list code was NOT refused; the bytes follow the layout THIS BUILD knows — verify against the deployed decoder before signing. |
| `contract_maker_pre_rest` | Info on ok maker-order (JIT): the maker has code and the derived pool does not exist — the EOA-only permit path is closed, so `data.execution.then` starts with create-pool + the cST→LOP (and, with enableJitMint, collateral→adapter) allowances from the account before the order rests. Silent when pool existence or maker code is unknown. |
| `maker_not_ready` | The MAKER side of a resting order cannot deliver as signed — the 2026-09-11 incident class (a contract maker's JIT order on a not-yet-created cST ranked #1: ERC-2612 permits are EOA-only, and the LOP's transfer helper counts a call to a CODE-LESS token as SUCCESS, so a fill "succeeds" moving nothing). Classified chain-side in `packages/core/src/handlers/maker-readiness.ts` (structural reasons vs recoverable fund gaps; `unknown` when no RPC or a read failed — never a verdict invented). On taker-fill: info on ok, `data.makerReadiness` carries the full verdict, the fix is the maker's. On a hybrid orderbook read: page-level info counting not-ready rows; each row carries `makerReadiness`, and the RANKED view excludes them (`exclusion: "maker-not-ready"` with the evidence under `whyNotFillable`) — excluded, never dropped, because every reason is fixable without re-signing. The readiness gate is what makes a green `fillSimulation` probe trustworthy: the silent-noop class simulates green. |
| `fill_sender_unknown` | Info on ok answer-rfq: the RFQ declares no `fill_sender` (venue 0.4.1 serves none yet), so the order is built OPEN — the LOP compares `allowedSender` with the address that CALLS it, and the requester account may fill through an adapter (a ForSelf integrator); a reservation is never guessed. Pass `fillSender` (the requester's own address when it calls the LOP itself, or its adapter) to reserve. The kernel's rule for the same case (2026-09-04). |
...
| `roles_not_granted` / `adapter_binding_mismatch` | JIT adapter pre-flight: controller roles missing (signable but unfillable; the role HOLDER is the adapter on the flat wire and the CorkMarketCreator on the nested wire — `readRoleHolder`) / on-chain bindings disagree with config (conflict — refresh cork-defaults.v2.json): flat = `adapter.MARKET_REGISTRY()`; nested = the CHAIN `adapter.MARKET_CREATOR()` → `creator.MARKET_REGISTRY()` plus one `POOL_MANAGER` end to end. Also for a ForSelf adapter whose CORK()/LOP()/WHITELIST() views disagree with config — no artifact built. A reverting `WHITELIST()` is NOT a conflict (a pre-caller-gate deployment; the pre-flight adapts). |
...
| `share_prediction_unavailable` | JIT prepare: the share simulation could not run or reverted — the message names the SIMULATED revert when there is one (`InvalidRate`, `RecipeRejectedConstraint`: the market the order would birth is one the fill rejects — fix the anchor/constraint, not the RPC) and eth_simulateV1-unsupported only when the transport is the fault (2026-09-22; the nested-fill rehearsal found the old generic message hiding a real revert). Verify the order side + permit token yourself when it fires. |
...
| `artifact_digest_mismatch` / `intent_hash_mismatch` / `venue_digest_mismatch` / `order_hash_mismatch` / `marketid_mismatch` / `create2_mismatch` | conflict: WHICH verification failed, branchable by code (split from the overloaded `digest_mismatch` 2026-08-10; the transitional "formerly digest_mismatch" wording was removed in 0.5.1). Track artifact digest / submit rollover: intent doesn't hash to its own `rolloverIntentHash` (not relayed) / venue echoed a different orderDigest / decode order + taker-fill: supplied or venue orderHash ≠ local recomputation. `order_hash_mismatch` also rides ok as INFO on an orderbook read: rows that do not hash to their own claimed orderHash are DROPPED chain-free (counted in `verification.dropped`) — a row misrepresenting its own order needs no RPC to refute. | On submit lop-order the venue may echo an orderHash: a value CONTRADICTING the local EIP-712 hash is a conflict with `accepted:false` — the local hash stays in `orderHash`/`localOrderHash` and the venue's rides as `venueOrderHash`; a case-only match or an omitted echo is clean.
...
| `settler_retired` | rollover prepare/submit: the settler belongs to a RETIRED rollover generation (a rollover block with a `retired` date — the arbitrum-v1.1 set's July 2026 stack, wire `rc.1`, retired by the rc.2 wire break) — venue-inadmissible AND wire-incompatible with live-generation digests; message names every ACTIVE replacement of the same kind across the chain's generations (`rolloverGenerationsOf`), the primary marked. Nothing built/relayed. |
| `settler_not_recognized` / `invalid_order_terms` | Info: settler not a configured Cork settler in ANY generation of the chain (`classifyAddress`; the teaching lists every active settler of that kind with its generation label). On dutch-auction-price and taker-fill it is a REFUSAL (unavailable) since 2026-08-26: an amount getter that is not the release-pinned current Fusion deployment cannot be priced here — the bytes after the address are caller-controlled data, not proof of what the contract charges — so no curve and no DERIVED cap. `data.classification` says which kind (`unknown`, or `legacy` under `phase_gated`). A taker-fill with an EXPLICIT `maximumTakingAmount` still builds and warns: the LOP enforces that cap on-chain, so an unknown getter can only make the fill revert, never overcharge. Also `invalid_order_terms` for incoherent order terms; covers a JIT fee over the target generation's rule — 5e18 inclusive on an 8-field pool manager, strictly below 100e18 on 10-field (Phoenix `InvalidFees()`; no `MAX_FEE_PERCENTAGE` getter exists on the 1.4.0 set). | Also on hybrid `rollover-orders` and track reconcile: a settler a venue row NAMES that is not a configured active/retired generation receives NO orderStatus read and NO log scan, and its row stays venue-provenance (`verification:"unverified"`, `settlerGeneration:"unknown"`) — reading an unrecognized contract would let it author the chain truth we then report (audit STATE-003).
...
| `implementation_not_approved` | Build-and-warn on ABI-typed prepares, a REFUSAL (`conflict`, `data.refused`, no bytes) on the JIT hook paths since 2026-09-03 (the bytes-decoder gate: the adapter DECODES bytes this tool ENCODES, so off-list code could misread them silently; `CORK_ALLOW_UNAPPROVED_CODE=1` / `--allow-unapproved-code` builds anyway, labeled `implementation_gate_bypassed`; unreadable code never refuses). Otherwise: the LIVE code behind a trusted role (corkAdapter, whitelistManager via its EIP-1967 slot, marketRegistry, jitAdapter, marketCreator — the `IMPLEMENTATION_ROLES`; the schema-1 `legacyJitAdapter`/`legacyMarketRegistry` role names are GONE, the legacy generation resolves through the same roles) hashes OFF the approved-implementations allowlist BUNDLED INTO THIS BUILD — a proxy upgrade nobody admitted (behavioral suite → allowlist entry), an address that moved ahead of a release, an empty account, or config drift. Addresses come from the resolved (remote-first) config; the allowlist never does, so a config that moves an address cannot admit the code behind it (audit MCP-NET-001). Scoped per prepare path to the roles its bytes execute: Phoenix `corkAdapter`+`whitelistManager`, prepare_market `marketRegistry`, JIT `jitAdapter`+`marketRegistry`, create-pool `marketCreator`+`marketRegistry` (`*_IMPLEMENTATION_ROLES` in implementations.ts); roles resolve WITHIN the generation in use (`implementationRoleAddress(role, generation)`), and the allowlist per chain per role is the UNION of every generation's live code hash because every generation's code is approved. `approved`/unreadable stay silent. Interface-first model: `packages/core/src/implementations.ts` + the cork-defaults `approvedImplementations` block. |
| `quote_ref_unverifiable` | conflict (submit lop-order): the cited RFQ option has no parsable positive premium — NOT relayed (deliberately STRICTER than the venue, which silently skips its band there). The party rule beside it (cork-api 0.4.1): the maker must be the RFQ requester OR the underwriter of the CITED answer (a maker-mode SELL citing its own quote); a rival's answer or a third party refuses `invalid_order_terms`; an identity the truncated embed cannot show defers to the venue (`citation_unresolved`). |
...
| `listing_traits_mismatch` | conflict (submit lop-order): listing fields (expiry/nonce/allowsPartialFills) contradict the SIGNED makerTraits [K3] — NOT relayed. Also INFO on an ok orderbook read: the venue's `allowedSender` echo contradicted the signed makerTraits on N rows — the served `allowedSender`/`exclusivity` are the local decode. |
| `private_order` | unavailable (taker-fill, raw + forSelf): the signed makerTraits reserve the fill for a sender whose LOW 80 BITS (last 10 bytes) differ from this fill's sender — the LOP reverts `PrivateOrder()`, so no bytes. The sender is `account` on the raw path and the ForSelf ADAPTER on the wrapper path. `data` names `allowedSender` (reserved suffix), `fillSender`, `fillSenderSuffix`. Chain-free (from the signed bytes). Maker side: `maker-order.allowedSender`; book side: `orderbook` rows' `allowedSender` + `exclusivity` (classified against `filters.account`). |
...
| `expiry_far_future` | Info on JIT maker-orders AND rollover jitMarket: `jitMarket.expiryTimestamp` >5 years out — cPT locked until expiry, and pool CREATION is bounded by the registry's maxExpiryDuration (30 days at last read; enforced by the JIT adapter, BaseFiller, and CorkMarketCreator — the old "no upper bound" claim was wrong for 2.1.0); check intent. The ladder + create-pool also pre-flight the bound itself → `would_revert` naming ExpiryOutOfRange. |
| `pool_already_exists` | Info on ok create-pool: the derived pool already exists — the tx is a safe idempotent no-op (re-runs the recipe checks, skips creation, returns the existing (poolId, cst, cpt)); creation-only checks (expiry bound, zero rate) are skipped. |
...
NAMESPACED (`ocoGroupNonce`, SDK `/orders`): the seed is `oco-group:<group>` and NO
clientRequestId may start with that prefix (`OCO_GROUP_NONCE_NAMESPACE`, refused by the schema
and by `buildMakerOrder`), so a group seed and an id seed are never the same string — the 40-bit
truncation any two seeds share stays the disclosed birthday risk (audit RC1-NONCE-001, 2026-09-11)
— and every order by the maker naming that group is one-cancels-the-other: the first
...
committed capture — re-capture deliberately with UPDATE_VENUE_SPEC=1. A
...
server-wide (429 + `Retry-After: 1`), and a 30 s deadline that is END-TO-END for the caller
(a 504 the moment it elapses, even mid-body-read) and whose `AbortSignal` rides `ctx.signal` into
the venue transport as `VenueDeps.signal` — composed with the per-call timeout, no call started
once aborted, the in-flight one aborted, reported as `request_aborted` and never fed to the
breaker (since 2026-09-11, audit DB-001; before that the signal was set on a field the transport
never read). The admission slot is released when the cancelled work settles, not at the
...
the WIRE each block speaks; the config DECLARES wires, the code IMPLEMENTS them and refuses a
declared wire it does not know (`phase_gated`) rather than guessing. A LABEL names the DISTRIBUTION BUNDLE (`cork/<version>`: `cork/v0.4`, `cork/v0.3`), never the core
protocol — Phoenix is one component of a bundle; the bundle's own record name (`phoenix/v0.4-rc.1`)
...
|---|---|---|---|---|---|
| 1 | `mainnet` (primary) | active | the original chain-1 stack, PM `0xccCC…B9eC` (8-field) | — | — |
| 42161 + 8453 | `cork/v0.4` (**primary**) | active | phoenix v1.4.0-rc.1: PM `0xcC17…0C2D`, corkAdapter `0x71eB…84A7`, whitelist `0x8aF6…EA51`, controller `0x6602…47b5` (10-field) | market-registry 0.5.0: registry `0xe1f5…55c5`, adapter `0x3E01…B104` (version() 0.4.0), creator `0x1A07…2DeA` (IN the registry package), recipes liquidity `0x679C…964d` / nav `0xed6A…87e3` / fixed `0xEC26…8C49` / impairment `0xd5e8…0Ed9`, owner `0x9fb6…b8d4` (nested) | rollover v0.2.0: factory `0x99A5…3F65`, exact `0x0F2C…452E`, partial `0x5E19…bE9e`, BaseFiller `0x3D16…1224`, seeded 503918966 / 51153216 (0.2) |
| 42161 + 8453 | `cork/v0.3` | active | phoenix v1.3.0-rc.1: PM `0x0280…7263`, corkAdapter `0xfa8A…72AD`, controller `0x6b65…82A1` (8-field) | market-registry 0.3.3: registry `0xa78d…11F1`, adapter `0x8902…374f`, creator `0x0aCc…586C` (cork-periphery 0.1.0), recipes `0xb881…Dc55` / `0xAeD3…f66d` / `0x133a…65C1` / impairment `0x7340…9eCA` (flat) | rollover v0.1.0-rc.2: factory `0x697A…5F82`, exact `0xF4ff…4C2f`, partial `0xC0fb…6B4e`, BaseFiller `0xCdD4…7B9d`, seeded 494104750 / 49917191 (rc.2) |
| 42161 | `arbitrum-v1.1` | active | the previous production stack where the venue's EXISTING markets live (176 pools; PM `0x4d0a…23d2`, corkAdapter `0xe9f3…1bed`) (8-field) | the pre-2.1.0 registry `0xF674…600A` + adapter `0xea15…8505` (legacy — reachable only with `CORK_ENABLE_DEPRECATED=1`) | the July 2026 set, factory `0xBBcC…E13A`, RETIRED 2026-08-13 (rc.1 — no jitMarketHash) |
| 42161 | `arbitrum-legacy` | read-only | the pre-launch calibration pair, PM `0xc2De…54AE`, 3 pools (8-field) | — | — |

Identical CREATE2 addresses across 42161 + 8453 in every Distribution set; bundler3 is per chain
(read from the adapter's own `BUNDLER3()`, Morpho infra, not a generation contract: `0x1FA4…8C13`
on 42161, `0x6BFd…20C4` on 8453). `forSelf.adapter` is the reference CorkForSelfAdapter of each
set (`0x3864…23f1` v0.2.0-rc.1 on the primary; `0x5Fc0…5771` v0.1.3-rc.1 on v0.3-rc.1).
Distribution component records verified 2026-09-22 (finalized Base 51171628/51176234/51291605/
51172961, Arbitrum 504060971/504097748/505010311/504074671); `contractsVersion` stays a free label
— the deployed getters (`adapter.version()` 0.4.0, `creator.version()` 0.1.0,
`registry.version()` 0.5.0) are recorded, never used as identity; the ADDRESS is the identity.

**Resolution rules.** (1) A PREPARE targets the primary unless the caller selects `generation:
<label>` (one optional field on every chain-backed input; `HandlerContext.generation` is the SDK
override, per-call beats ctx); an unknown label refuses `generation_unknown` listing the chain's
labels, a read-only set refuses a pre-expiry prepare `generation_read_only`. (2) A POOL-scoped read
or prepare by poolId (cork-pool, account-state, pool-whitelist, the 13 phoenix actions + ForSelf
twins, the three chain compute kinds, track marketRef) resolves the generation FROM THE CHAIN —
`resolvePoolGeneration`: one batched `shares(poolId)` read across every generation's pool manager,
the manager that knows the pool wins, `generation` narrows to one; none knows it → `pool_not_found`
naming every manager asked; all THROW → `chain_read_failed` (the breaker still learns). Funding
legs, preflight, sweep, approvals, decode-trust targets, bundler3 and corkAdapter all come from the
POOL's block — a bundle for a v1.1 pool targets the v1.1 adapter. Every such result carries
`data.generation` and `provenance.generation`. Why this matters: every pool the venue serves today
lives on an older pool manager than the 10-field primary (2026-09-22 venue counts: 453 on Arbitrum
= 176 arbitrum-v1.1 + 3 arbitrum-legacy + 274 cork/v0.3; 466 on Base, all cork/v0.3;
none yet on `0xcC17…`), so a read routed through the
primary's addresses was wrong for all of them. (3) ADDRESS-scoped classification (an adapter,
settler, factory, pool manager seen in bytes/logs/venue rows) is ONE function, `classifyAddress` →
`{ label, status, role }` over `GENERATION_ROLES` (poolManager, constraintAdapter, corkAdapter,
whitelistManager, controller, registry, jitAdapter, marketCreator, factory, exactSettler,
partialSettler, baseFiller, forSelfAdapter, recipe) — decode labels (`jit.generation` +
`jit.wire`; the literal `"2.1.0"`/`"legacy (pre-2.1.0)"` strings are gone), event attribution,
settler classification, the book's row verification and `unknown_target` all read it. Decode
dispatches a JIT hook by the adapter's CLASSIFICATION (its generation's wire), never by trial
decoding — nested bytes at a flat adapter get no label. `generationsOf(defaults, chainId)` is the
flattening (primary → active → read-only); `primaryOf`, `selectGeneration(list, label, purpose)`,
`marketRegistryForWire`, `rolloverGenerationsOf` (the rollover flattening every settler consumer
reads: the primary's block, other live blocks, retired blocks last, each labeled with ONE `primary`
flag) sit beside it. Mutation-probed (`gen-*`, `nested-*`, `rollover02-*`, `poolgen-*`).

**The wire table — codecs live in ONE place per wire (2026-09-22; golden vectors FROM THE CHAIN,
captured on 42161: the adapter's own `encodeExtraData` bytes reproduced byte-for-byte and decoded
back, `pm.getId` for the 10-field id, the creator/verify/deploy/controller selectors + word layouts,
BaseFiller's `hashJITMarketParams` `0xcaaaf169…1c62`).**

| Wire | Block | Shape | Where |
|---|---|---|---|
| `8-field` | phoenix | `Market` has 8 fields; poolId = keccak(abi.encode(8)); fees OUTSIDE the id (`swapFee`/`unwindSwapFee` views); `MarketCreated` 7 args (`0x0dac57f1…`); fee cap 5e18 INCLUSIVE (`MAX_FEE_PERCENTAGE`); controller `createNewPool(pool, unwindFee, swapFee, isWhitelistEnabled)` | `marketid.ts` `Market8`, `chain/reads.ts` `marketAbiFor` |
| `10-field` | phoenix | fees INSIDE `Market` AND the id; `MarketCreated` 9 args (`POOL_MANAGER_MARKET_CREATED_10_TOPIC`); fee rule strictly `< 100e18` — Phoenix `InvalidFees()`, NO `MAX_FEE_PERCENTAGE` getter (the constants-cache refresh of it runs only under flat); controller `createNewPool((Market, isWhitelistEnabled))`; the views still exist and are compared with the tuple (`feeDisagreementWarnings`) | `Market10`, `computeMarketId(market, wire)` refuses a shape that contradicts the wire (`assertMarketWire`) |
| `legacy` | marketRegistry | pre-2.1.0 mode strings, constraint derived at FILL time; deprecation gate (`legacy:true` + `CORK_ENABLE_DEPRECATED=1`) | `market-registry-legacy.ts` |
| `flat` | marketRegistry | 0.3.x: `abi.encode(JITMarketParams{9 fields + enableJitMint LAST}, PermitParams[])` with `additionalData`; `verify(ca, ref, oracle, constraint, additionalData)` (5); `deploy(ca, ref, mode)` (3); creator = cork-periphery 0.1.0; roles on the ADAPTER; `adapter.MARKET_REGISTRY()` binding; adapter emits `JITMarketCreated` (6 args, recipe address); byte-identical to 0.5.1 | `market-registry.ts` `WIRES.flat` |
| `nested` | marketRegistry | 0.5.0: `abi.encode((MarketParams market, bool enableJitMint), PermitParams[])`, MarketParams = `(ca, ref, expiry, recipe, rateOverride, constraint(4), bytes extraData, bytes32 oracleSalt, swapFee, unwindFee)` — salt at index 7 between the bytes and the fees; `verify(ca, ref, oracle, expiryTimestamp, creating, constraint, extraData)` (7, selector `0x15bb9583`); `deploy(ca, ref, mode, oracleSalt)` (4, `0x5475abdc`); `wrapperKey`; creator `createNewPool(MarketParams) → (poolId, cst, cpt)` (`0x59c8eb4c`) IN the registry package; binding CHAIN `adapter.MARKET_CREATOR() → creator.MARKET_REGISTRY()` + one `POOL_MANAGER` end to end (`adapter.LIMIT_ORDER_PROTOCOL/POOL_MANAGER/MARKET_CREATOR`, `creator.CONTROLLER/MARKET_REGISTRY/POOL_MANAGER`); POOL_CREATOR_ROLE on the CREATOR (the adapter holds none; no FEE_MANAGER_ROLE on the 1.4.0 controller — `readRoleHolder` probes by wire); the adapter emits NO `JITMarketCreated` — creation is announced by the creator's `MarketCreated` (9 args, `caller` indexed, `CREATOR_MARKET_CREATED_TOPIC`); denominations are ADDRESS units (`isDenomination`, `getDenominations → address[]`), feeds `(base, quote, aggregatorAddress)` | `WIRES.nested`, `encodeJitExtraData(wire, …)`, `IMPLEMENTED_MARKET_REGISTRY_WIRES = [flat, nested]` |
| `rc.1` | rollover | the July 2026 OrderData (832 bytes, no `jitMarketHash`); refuses a jitMarket | `rollover.ts` |
| `rc.2` | rollover | `RolloverParams` gained a trailing `bytes32 jitMarketHash` (OrderData 864 bytes); `JITMarketParams(…,bytes additionalData,uint256 swapFeePercentage,uint256 unwindSwapFeePercentage,bool enableJitMint)` typehash; forge golden unchanged | `hashJitMarketParams(p, "rc.2")` |
| `0.2` | rollover | `bytes32 oracleSalt` inserted AFTER `additionalData` in `JITMarketParams`; the typehash changes (`JIT_MARKET_PARAMS_TYPEHASHES`); OrderData/RolloverParams IDENTICAL to rc.2 (CorkSettler/1.0.0 domain unchanged, `eip712Domain()` verified both settlers both chains); BaseFiller emits the 3-arg `JITMarketCreated(bytes32,address,address)` (`BASE_FILLER_JIT_MARKET_CREATED_TOPIC`, emitter role `baseFiller`) | `hashJitMarketParams(p, "0.2")`; Solidity mirror `experiments/fork-harness/src/RolloverJitHashMirror02.sol` (forge 3/3) |

Rollover typed-data OUTPUT keeps the struct's own `additionalData` member name (the signer sees
what the contract hashes); every jitMarket INPUT says `extraData` (alias `additionalData` +
`deprecation_notice`; both present and different refuse). `oracleSalt` defaults to zero in ONE
resolver, is disclosed as mattering only on a pair's FIRST oracle deploy, and refuses non-zero on
a flat/legacy/rc.2 target naming the generation. The rollover-intent prepare takes the wire from
the SETTLER's generation, binds THAT generation's registry + fee rule, derives the destination pool
with the 10-field identity under 0.2 (`deriveRolloverJitPool`), echoes `jitMarketWire`. Event
scans decode each log with the ABI of its EMITTER's declared wire (`event-attribution.ts`; a
parity test holds every verified Cork topic to at least one emitter role).

**History that still explains the model.** The 0.3.3 redeploy (2026-08-10) fixed the CREATE2
collision: the wrapper key doubles as the CREATE2 salt and is keccak256(abi.encode(**registryAddress**,
ca, ref, caSource, refSource)) — every registry derives its own salt space, so the sUSDe/sUSDS@42161
brick cannot recur (verified live: the pair simulates DEPLOYABLE on 0.3.3, still reverts on the
old); the 0.3.2 set `0xF532…DC94` is superseded, git history keeps its record; predicted wrapper
addresses come from simulateContract(registry.deploy) — on-chain, never a local salt port — so no
salt change ever needed math changes. The 2.1.0 MODEL (recipes are approved CONTRACTS that
resolve/verify their own constraints; the constraint is derived OFF-CHAIN at signing
(recipe-rate-constraint) and CARRIED in the order — pool id + share addresses pinned at signing, no
rate-driven identity drift; oracles mode-keyed price|nav per pair plus fixed-rate keyed on the
rate; scales 1e18 = 1.0 except `_PERCENTAGE`-named recipe constants and the two fee fields at
1e18 = 1%) holds on BOTH the flat and the nested wire. The PREVIOUS generation is dangerous
precisely because it still ANSWERS: newer-shaped calls against it decode into plausible nonsense —
the binding guard (`adapter.MARKET_REGISTRY()` on flat, the `MARKET_CREATOR → MARKET_REGISTRY`
chain on nested; best-effort+cached on reads, hard on prepares → `adapter_binding_mismatch`) and
`computeMarketId(market, wire)` (refuses a shape that contradicts the wire — viem decodes a
10-field `market()` through an 8-field ABI WITHOUT error and yields a wrong poolId silently) are
the two tripwires. The rc.2 wire break (2026-08-13) retired the July 2026 rollover set (venue-
archived; kept on `arbitrum-v1.1` for event-history scans + the `settler_retired` teaching:
rc.2-typed digests do not verify on it). Since 2026-09-11 the venue (cork-indexing-api 0.4.2)
admits EVERY non-archived rollover factory and each factory approves only ITS OWN settlers, so the
mode-mismatch teaching names the same generation's partner; rc.2 and 0.2 run side by side on both
chains. Older 0.5.x binaries validate the FROZEN schema-1 file and never see the new set (a
warn-and-relay path, never a refusal). The registry read API dependency was REMOVED 2026-08-12 —
the live parity suite (`rpc-live.test.ts`, CORK_RPC_LIVE=1, `live-smoke`) compares our chain-native
reads against an INDEPENDENT in-test raw-read reference (own minimal ABIs, one-shot enumeration
vs pagination, `predictFixedRateOracle` + code-existence vs our deploy simulation, raw
`recipe.resolve` wei-for-wei, an independent Market-tuple re-encode of the poolId, label→labelHash
re-hashing); `CORK_MARKET_API` is gone. Share prediction works pre-oracle-deploy (the simulation
prepends the permissionless deploy). The venue's registry v2 `POST /registry/v2/{chain}/market/predict`
REQUIRES `oracle_salt`, `swap_fee_percentage`, `unwind_swap_fee_percentage` (strings) and a string
`expiry`; live RFQs cite `market_template.market_template_id` (opaque) and the venue exposes NO
template-resolution route, so answer-rfq reads only the inline block — under a nested generation
the inline schemas `cork-inline-liquidity/1` / `cork-inline-impairment/1` gain an OPTIONAL
`oracle_salt` (default zero; a requester↔underwriter convention, ours; raise with the venue owner).

**Proven end-to-end on forks (private tree, `experiments/fork-harness/`).** The flat 2.1.0 fill
path: `test/JitOrderRoundTrip210.t.sol` — tool-prepared order + embedded cST permit filled through
the real 1inch LOP on an Arbitrum fork, oracle deployed in-fill, pool created at the derived id,
cST exactly equal to the prediction, plus a negative control proving an out-of-window constraint
reverts RecipeRejectedConstraint. The nested path: `script/nested-fill-rehearsal.ts` (Base fork
block 51643259, 2026-09-22) — the same tool pipeline (`runTool`) against the primary: derive →
maker-order + nested jitMarket (`extraDataLayout: verified-on-chain`, the deployed adapter's
...
`InvalidRate` from the pool manager once the live 1.0901 rate is read) — and the old generic
`share_prediction_unavailable` hid that revert (fixed: the message names the simulated revert).
Encoder ↔ EVM parity: `src/JitExtraDataDecoder.sol` holds BOTH decoders (flat + nested;
`test/JitExtraDataDecoder.t.sol` reads `test/fixtures/jit-extra-data-nested.json` and proves flat
bytes revert in the nested decoder); `src/RolloverJitHashMirror02.sol` mirrors BaseFiller 0.2's
`hashJITMarketParams` (`test/RolloverJitHashMirror02.t.sol`). A real mainnet pool for
...
vnet — chainId 1 without a vnet RPC yields `pool_not_found`, by design.
...
forSelf? }` and every block declares its `wire` (`8-field|10-field`, `legacy|flat|nested`,
`rc.1|rc.2|0.2`) — see "Per-chain coverage" for the table and the wire semantics. `lopAddresses`
and `fusionSettlements` are unchanged from schema 1. The `$…Comment` strings carry the verified
provenance per block (deploy blocks, seeds, roles, what was read live and when). Resolvers:
`resolveGenerations(chainId)`; `resolveDeployment` / `resolveRollover` / `resolveMarketRegistry`
keep their names + return shapes and ADD `generation: { label, status, wire }` (optional
`generation` label argument); `resolveMarketRegistryLegacy` is GONE — the legacy lane is the
generation whose registry wire is `legacy`, and the deprecation gate keys on the wire.
`getDep(ctx, chainId, { purpose, generation })` selects for a prepare; `getPoolDep` →
`resolvePoolGeneration` for anything keyed on a poolId. Attestations (`CREATE2_ATTESTATIONS`,
config.ts) BIND a generation (`binds: { section, generation, chains, path }`) and may pin a
`commit` as source; `topic:"verify"` re-derives them — the `mainnet` and `cork/v0.3`
sets are attested; the `cork/v0.4` set has NO attestation because the Distribution
component records carry no salt/initCodeHash/deployer (nothing fabricated; its live code hashes
are in `approvedImplementations` instead).

`cork-defaults.json` (schema 1) is FROZEN for the 0.5 line and keeps its CURRENT primary addresses
(cork/v0.3). WHY it cannot simply move: a schema-1 file whose primary moved would send
0.5.x binaries to a generation whose wire they do not speak — viem decodes a 10-field `market()`
return through an 8-field ABI WITHOUT error, so the wrong poolId would be silent. Neither line can
serve the other's shape; the 0.6 bundled copy is v2 and a test pins that the frozen v1 file still
parses under a copy of the v1 schema kept in the test (never in src). Older binaries `.strip()`
unknown keys, so a NEWER v1 key (e.g. `activeGenerations`) is a warning path there, never a refusal.
...
Roles are `IMPLEMENTATION_ROLES` = corkAdapter, whitelistManager (proxy: eip1967), marketRegistry,
jitAdapter, marketCreator — the schema-1 `legacyJitAdapter`/`legacyMarketRegistry` names are gone;
per chain per role the list is the UNION of every generation's live code hash (captured 2026-08-12
for 0.3.3 / v1.3.0-rc.1 / legacy and 2026-09-22 for 0.5.0 / v1.4.0-rc.1, each cross-checked against
the Distribution record's codeHash) because every generation's code is approved, and the guard
checks the code behind the address of the generation IN USE (`implementationRoleAddress(role,
generation)` resolves WITHIN the selected generation — no address is duplicated in the block).
Adding a role means: the address in its generation block, the live `keccak256(eth_getCode)` in
...
(math ports, chain reads, Bundler3 encode/decode, remote config + the generation model `generations.ts`, `runTool` dispatch — handlers
...
`/indexer` `/config`) map 1:1 to barrels in `packages/core/src/exports/` (`/config` = config.ts + config-remote.ts + generations.ts + implementations.ts + phala-attest.ts + version.ts — the generation model is public SDK surface: `generationsOf`, `selectGeneration`, `classifyAddress`, `resolvePoolGeneration`, `rolloverGenerationsOf`, the wire enums and block schemas) — package.json exports
